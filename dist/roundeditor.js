...
function il(e, { map: t, tableStart: n, table: r }, i) {
	let a = i > 0 ? -1 : 0;
...
function sl(e, { map: t, table: n, tableStart: r }, i) {
...
function cl(e, t) {
	...
	if (t) {
		...
		for (let e = n.right - 1; sl(r, n, e), e != n.left; e--) {
...
function ul(e, { map: t, tableStart: n, table: r }, i) {
	let a = n;
	for (let e = 0; e < i; e++) a += r.child(e).nodeSize;
	let o = [], s = i > 0 ? -1 : 0;
	ll(t, r, i + s) && (s = i == 0 || i == t.height ? null : 0);
...
function fl(e, t) {
	...
	if (t) {
		let n = rl(e);
		t(ul(e.tr, n, n.bottom));
	}
	return !0;
}
function pl(e, { map: t, table: n, tableStart: r }, i) {
	let a = 0;
...
function ml(e, t) {
	...
	if (t) {
		...
		for (let e = n.bottom - 1; pl(r, n, e), e != n.top; e--) {
			let e = n.tableStart ? r.doc.nodeAt(n.tableStart - 1) : r.doc;
			if (!e) throw RangeError("No table found");
...
function gl({ width: e, height: t, map: n }, r) {
...
function _l(e, t) {
	...
	if (t) {
		...
		if (n.setNodeMarkup(c + r.tableStart, null, {
			...
			let e = c + 1 + l.content.size, t = hl(l) ? c + 1 : e;
...
function vl(e, t) {
	let n = W(e.schema);
	return yl(({ node: e }) => n[e.type.spec.tableRole])(e, t);
}
function yl(e) {
	return (t, n) => {
...
function bl(e) {
	return function(t, n) {
		if (!zc(t)) return !1;
		if (n) {
			let r = W(t.schema), i = rl(t), a = t.tr, o = i.map.cellsInRect(e == "column" ? {
				left: i.left,
				top: 0,
				right: i.right,
				bottom: i.map.height
...
function Sl(e, t) {
	return t = t || { useDeprecatedLogic: !1 }, t.useDeprecatedLogic ? bl(e) : function(t, n) {
		if (!zc(t)) return !1;
		if (n) {
			let r = W(t.schema), i = rl(t), a = t.tr, o = xl("row", i, r), s = xl("column", i, r), c = (e === "column" ? o : e === "row" && s) ? 1 : 0, l = e == "column" ? {
				left: 0,
				top: c,
				right: 1,
				bottom: i.map.height
...
Sl("row", { useDeprecatedLogic: !0 }), Sl("column", { useDeprecatedLogic: !0 }), Sl("cell", { useDeprecatedLogic: !0 });
function Cl(e, t) {
...
function Tl(e) {
	...
	if (a == "row") for (let e = 0; e < t.childCount; e++) {
		...
		(a || c) && (i = Dl(W(o).row, new d(i, a, c)).content), s.push(i);
	}
	else if (a == "cell" || a == "header_cell") s.push(n || r ? Dl(W(o).row, new d(t, n, r)).content : t);
	else return null;
	return El(o, s);
}
function El(e, t) {
...
function Ol({ width: e, height: t, rows: n }, r, i) {
	if (e != r) {
		let t = [], i = [];
...
function Ml(e, t, n, r, i) {
	...
	kl(f, o, a, n, l, u, p) && m(), Al(f, o, a, n, c, l, s, p) && m(), Al(f, o, a, n, c, l, u, p) && m(), jl(f, o, a, n, s, u, c, p) && m(), jl(f, o, a, n, s, u, l, p) && m();
	for (let e = s; e < u; e++) {
...
var Nl = Fr({
	...
	ArrowUp: Fl("vert", -1),
	ArrowDown: Fl("vert", 1),
	"Shift-ArrowLeft": Il("horiz", -1),
	"Shift-ArrowRight": Il("horiz", 1),
	"Shift-ArrowUp": Il("vert", -1),
	"Shift-ArrowDown": Il("vert", 1),
	Backspace: wl,
	"Mod-Backspace": wl,
	Delete: wl,
	"Mod-Delete": wl
...
function Fl(e, t) {
	return (n, r, i) => {
		if (!i) return !1;
		let a = n.selection;
		if (a instanceof G) return Pl(n, r, C.near(a.$headCell, t));
		if (e != "horiz" && !a.empty) return !1;
		let o = Bl(i, e, t);
		if (o == null) return !1;
		if (e == "horiz") return Pl(n, r, C.near(n.doc.resolve(a.head + t), t));
		{
			let i = n.doc.resolve(o), a = Wc(i, e, t), s;
			return s = a ? C.near(a, 1) : t < 0 ? C.near(n.doc.resolve(i.before(-1)), -1) : C.near(n.doc.resolve(i.after(-1)), 1), Pl(n, r, s);
		}
	};
}
function Il(e, t) {
	return (n, r, i) => {
		...
		return s ? Pl(n, r, new G(o.$anchorCell, s)) : !1;
	};
}
function Ll(e, t) {
...
function Rl(e, t, n) {
	...
	let r = Tl(n), i = e.state.selection;
	if (i instanceof G) {
		r || (r = {
			width: 1,
			height: 1,
			rows: [a.from(Dl(W(e.state.schema).cell, n))]
		});
		let t = i.$anchorCell.node(-1), o = i.$anchorCell.start(-1), s = U.get(t).rectBetween(i.$anchorCell.pos - o, i.$headCell.pos - o);
		return r = Ol(r, s.right - s.left, s.bottom - s.top), Ml(e.state, e.dispatch, o, s, r), !0;
	}
	if (r) {
		let t = Bc(e.state), n = t.start(-1);
		return Ml(e.state, e.dispatch, n, U.get(t.node(-1)).findCell(t.pos - n), r), !0;
	}
	return !1;
}
function zl(e, t) {
	if (t.button != 0 || t.ctrlKey || t.metaKey) return;
	let n = Vl(e, t.target), r;
	if (t.shiftKey && e.state.selection instanceof G) i(e.state.selection.$anchorCell, t), t.preventDefault();
	else if (t.shiftKey && n && (r = Lc(e.state.selection.$anchor)) != null && Hl(e, t)?.pos != r.pos) i(r, t), t.preventDefault();
	else if (!n) return;
	function i(t, n) {
		let r = Hl(e, n), i = Ic.getState(e.state) == null;
		if (!r || !Uc(t, r)) {
			if (i) r = t;
			else return;
		}
	...
	function o(r) {
		...
		else if (Vl(e, o.target) != n && (c = Hl(e, t), !c)) return a();
		c && i(c, o);
	}
...
var Ul = class {
	constructor(e, t) {
		this.node = e, this.defaultCellMinWidth = t, this.dom = document.createElement("div"), this.dom.className = "tableWrapper", this.table = this.dom.appendChild(document.createElement("table")), this.table.style.setProperty("--default-cell-min-width", `${t}px`), this.colgroup = this.table.appendChild(document.createElement("colgroup")), Wl(e, this.colgroup, this.table, t), this.contentDOM = this.table.appendChild(document.createElement("tbody"));
	}
	update(e) {
		return e.type == this.node.type && (this.node = e, Wl(e, this.colgroup, this.table, this.defaultCellMinWidth), !0);
	}
	ignoreMutation(e) {
...
var K = new zn("tableColumnResizing");
function Gl({ handleWidth: e = 5, cellMinWidth: t = 25, defaultCellMinWidth: n = 100, View: r = Ul, lastColumnResizable: i = !0 } = {}) {
	let a = new In({
		key: K,
		state: {
			init(e, t) {
				...
				return r && o && (o[s] = (e, t) => new r(e, n, t)), new Kl(-1, !1);
			},
			apply(e, t) {
				return t.apply(e);
			}
		...
		props: {
			...
			decorations: (e) => {
				let t = K.getState(e);
				if (t && t.activeHandle > -1) return iu(e, t.activeHandle);
			},
			nodeViews: {}
		}
	});
...
function ql(e, t, n, r) {
	...
	let i = K.getState(e.state);
	if (i && !i.dragging) {
		let a = Zl(t.target), o = -1;
		if (a) {
			let { left: r, right: i } = a.getBoundingClientRect();
			t.clientX - r <= n ? o = Ql(e, t, "left", n) : i - t.clientX <= n && (o = Ql(e, t, "right", n));
		}
		if (o != i.activeHandle) {
			if (!r && o !== -1) {
...
function Jl(e) {
	if (!e.editable) return;
	let t = K.getState(e.state);
	t && t.activeHandle > -1 && !t.dragging && eu(e, -1);
}
function Yl(e, t, n, r) {
	if (!e.editable) return !1;
	let i = e.dom.ownerDocument.defaultView ?? window, a = K.getState(e.state);
	if (!a || a.activeHandle == -1 || a.dragging) return !1;
	let o = e.state.doc.nodeAt(a.activeHandle), s = Xl(e, a.activeHandle, o.attrs);
	e.dispatch(e.state.tr.setMeta(K, { setDragging: {
		startX: t.clientX,
		startWidth: s
	} }));
	function c(t) {
		i.removeEventListener("mouseup", c), i.removeEventListener("mousemove", l);
		let r = K.getState(e.state);
		r?.dragging && (tu(e, r.activeHandle, $l(r.dragging, t, n)), e.dispatch(e.state.tr.setMeta(K, { setDragging: null })));
	}
	function l(t) {
		if (!t.which) return c(t);
		let i = K.getState(e.state);
		if (i && i.dragging) {
			let a = $l(i.dragging, t, n);
			nu(e, i.activeHandle, a, r);
		}
	}
	return nu(e, a.activeHandle, s, r), i.addEventListener("mouseup", c), i.addEventListener("mousemove", l), t.preventDefault(), !0;
}
function Xl(e, t, { colspan: n, colwidth: r }) {
...
function eu(e, t) {
	e.dispatch(e.state.tr.setMeta(K, { setHandle: t }));
}
function tu(e, t, n) {
	...
	for (let e = 0; e < a.height; e++) {
		...
		let d = l.colwidth ? l.colwidth.slice() : ru(l.colspan);
...
function iu(e, t) {
	...
	for (let t = 0; t < a.height; t++) {
		...
		if ((s == a.width - 1 || a.map[r] != a.map[r + 1]) && (t == 0 || a.map[r] != a.map[r - a.width])) {
			...
			c.className = "column-resize-handle", K.getState(e)?.dragging && n.push(R.node(o + t, o + t + i.nodeAt(t).nodeSize, { class: "column-resize-dragging" })), n.push(R.widget(s, c));
		}
	}
	return z.create(e.doc, n);
}
function au({ allowTableNodeSelection: e = !1 } = {}) {
	return new In({
		key: Ic,
		state: {
			init() {
...
function uu(e, t, n = 0) {
	...
	typeof i?.loadFilelist == "function" ? (r.data("editorStatus", t), i.loadFilelist(r, !0)) : n < 20 && window.setTimeout(() => uu(e, t, n + 1), 100);
}
function du(e, t, n = () => {}) {
	return new Promise((r, i) => {
		let a = new XMLHttpRequest();
		a.open("POST", ou()), a.responseType = "json", a.withCredentials = !0, a.upload?.addEventListener("progress", (e) => {
			...
			t.download_url = su(t.download_url), t.source_filename = su(t.source_filename), t.thumbnail_filename = lu(t.thumbnail_filename), uu(e.sequence, t), r(t);
...
async function fu(e, t, n = () => {}) {
	...
	for (let [i, a] of t.entries()) {
		let t = await du(e, a.file || a, (e) => n(i, e));
		r.push({
			...t,
...
var pu = new zn("roundeditor-upload-placeholders");
function mu(e) {
	let t = Number(e);
	return Number.isFinite(t) ? Math.min(1, Math.max(0, t)) : 0;
}
function hu(e, t, n = 0) {
	let r = Math.round(mu(n) * 100), i = document.createElement("span");
...
function gu(e, t) {
	return R.widget(e, () => hu(t.type, t.label, t.progress), {
		...t,
		side: -1
	});
}
function _u() {
	return new In({
		...
		state: {
			init: () => z.empty,
			apply(e, t) {
				let n = t.find(), r = t.map(e.mapping, e.doc), i = e.getMeta(pu), a = new Set(r.find().map((e) => e.spec.id)), o = n.flatMap((t) => a.has(t.spec.id) ? [] : [gu(Math.min(e.doc.content.size, Math.max(0, e.mapping.map(t.from, 1))), t.spec)]);
				if (o.length && (r = r.add(e.doc, o)), i?.add) {
					let { position: t, ...n } = i.add;
					r = r.add(e.doc, [gu(t, n)]);
				}
				if (i?.update) {
					let t = r.find(null, null, (e) => e.id === i.update.id)[0];
					if (t) {
						let n = {
							...t.spec,
							progress: mu(i.update.progress),
							label: i.update.label ?? t.spec.label
						};
						r = r.remove([t]), r = r.add(e.doc, [gu(t.from, n)]);
					}
				}
				return i?.remove && (r = r.remove(r.find(null, null, (e) => e.id === i.remove.id))), r;
			}
		},
		props: { decorations: (e) => pu.getState(e) }
	});
}
function vu(e, t, n, r = e.state.selection.from) {
	let i = {};
	return e.dispatch(e.state.tr.setMeta(pu, { add: {
		id: i,
		position: r,
		type: t,
		label: n,
...
function yu(e, t, n, r = null) {
	e.dispatch(e.state.tr.setMeta(pu, { update: {
		id: t,
		progress: n,
		label: r
	} }));
...
function bu(e, t) {
	return pu.getState(e)?.find(null, null, (e) => e.id === t)[0]?.from ?? null;
}
function xu(e, t) {
	e.dispatch(e.state.tr.setMeta(pu, { remove: { id: t } }));
}
function Su(e, t) {
	return e.setMeta(pu, { remove: { id: t } });
}
//#endregion
//#region src/mediaInsertion.js
var Cu = 2, wu = 2, Tu = /* @__PURE__ */ new Set([
	"image",
	"audio",
	"video"
]);
...
function ku(e, t) {
	...
	let s = Math.max(0, Cu - r);
	return s && (e = e.insert(i, a.fromArray(Ou(e.doc.type.schema, s)))), e.setSelection(w.create(e.doc, t + 1));
}
function Au(e, t) {
	let n = t[t.length - 1], r = n ? Eu(e.doc, n) : null;
	if (r === null) return e;
	let i = r + n.nodeSize;
	...
	return s?.isTextblock ? ku(e, o + s.nodeSize) : e;
}
function ju(e, t) {
	let n = e.state.doc.nodeAt(t), r = e.state.doc.resolve(t), i = null, o = null;
	for (let t = r.depth; t > 0; t--) if (r.node(t).type === e.state.schema.nodes.paragraph) {
		i = r.before(t), o = t;
		break;
	}
	if (i === null) return !1;
	let s = e.state.tr;
	if (o !== null && r.index(o) > 0) {
		if (!Ht(s.doc, t)) return !1;
		s = s.split(t);
		let e = n ? Eu(s.doc, n) : null;
		if (i = e === null ? null : Du(s.doc, e), i === null) return !1;
	}
	return s = s.insert(i, a.fromArray(Ou(e.state.schema, wu))), e.dispatch(s.setSelection(w.create(s.doc, i + 1)).scrollIntoView()), e.focus(), !0;
}
function Mu(e) {
	let t = e;
	for (; t?.childCount;) t = t.lastChild;
	return !!(t && Tu.has(t.type.name));
}
function Nu(e, t) {
	let n = e.resolve(t), r = null;
	for (let e = n.depth; e > 0; e--) if (n.node(e).type.name === "paragraph") {
		r = e;
		break;
	}
	if (r === null) return !1;
	let i = n.node(r), a = n.index(r);
	if (a > 0) return Tu.has(i.child(a - 1).type.name);
	let o = n.before(r);
	return o === 0 || Mu(e.resolve(o).nodeBefore);
}
//#endregion
...
function Lu(e, t, { position: n = null, align: r = null, placeholderId: i = null, insertionMode: o = "paragraph" } = {}) {
	if (!t.length) return !1;
	let { state: s } = e.view, c = t.map((e) => s.schema.nodes.image.create(Fu(e))), l = o === "paragraph" ? c.map((e) => s.schema.nodes.paragraph.create(null, e)) : c, u = new d(a.fromArray(l), 0, 0), f = s.tr, p = i ? bu(s, i) : null, m = p ?? (n === null ? s.selection.from : n), h = Iu(s.doc, m);
	...
	return i && (f = Su(f, i)), o === "paragraph" && (f = Au(f, c)), e.view.dispatch(f.scrollIntoView()), e.view.focus(), !0;
}
function Ru(e, t, n = null) {
	let r = vu(e.view, "image", e.config.labels?.imageUploading || "Uploading image…", n ?? e.view.state.selection.from);
	fu(e, t.map((e) => ({ file: e }))).then((t) => Lu(e, t, {
		position: n,
		placeholderId: r
	})).catch((t) => {
		xu(e.view, r), window.alert?.(t.message);
	});
}
function zu(e, t) {
	if (!e.config.allowUpload) return !1;
	let n = Pu(t.clipboardData?.files);
	return n.length ? (t.preventDefault(), Ru(e, n), !0) : !1;
}
//#endregion
//#region src/icons.js
var Bu = "/modules/editor/skins/roundeditor/assets/attachment-icons.svg", Vu = {
...
function q(e) {
	...
	return n.setAttribute("href", `${Bu}#${e}`), t.appendChild(n), t;
}
function Uu(e) {
	let t = document.createElementNS("http://www.w3.org/2000/svg", "svg");
	return t.classList.add("roundeditor__icon", `roundeditor__icon--${e}`), Hu[e] && t.classList.add(Hu[e]), t.dataset.icon = e, t.setAttribute("viewBox", "0 0 24 24"), t.setAttribute("aria-hidden", "true"), t.setAttribute("focusable", "false"), t.innerHTML = Vu[e] || Vu.help, t;
}
//#endregion
...
function Ku(e) {
	...
	return Wu.has(t) || Gu.test(String(e?.name || ""));
}
function qu(e) {
	return Array.from(e || []).filter(Ku);
}
function Ju(e) {
	if (!Number.isFinite(e) || e <= 0) return "";
	let t = Math.max(1, Math.round(e)), n = Math.floor(t / 3600), r = Math.floor(t % 3600 / 60), i = t % 60, a = `${r}:${String(i).padStart(2, "0")}`;
	return n ? `${n}:${String(r).padStart(2, "0")}:${String(i).padStart(2, "0")}` : a;
}
function Yu(e) {
	return e === "center" ? {
		align: e,
...
function Xu(e, t = Infinity, n = null) {
	let r = String(e.original_type || "").toLowerCase() === "image/gif";
	return {
		src: cu(e.download_url),
		poster: lu(e.thumbnail_filename) || null,
		caption: "",
		width: null,
		height: null,
		displayWidth: null,
...
function Zu(e, t, { align: n = null, position: r = null, placeholderId: i = null, insertionMode: a = "paragraph" } = {}) {
	let { state: o } = e.view, s = o.schema.nodes.video.create(Xu(t, Infinity, n)), c = a === "paragraph" ? o.schema.nodes.paragraph.create(null, s) : s, l = i ? bu(o, i) : null, u = l !== null || r !== null ? o.tr.replaceRangeWith(l ?? r, l ?? r, c) : o.tr.replaceSelectionWith(c);
	return i && (u = Su(u, i)), a === "paragraph" && (u = Au(u, [s])), e.view.dispatch(u.scrollIntoView()), e.view.focus(), !0;
}
async function Qu(e, t, n = () => {}) {
	return {
		...await du(e, t.file || t, n),
		dimensions: t.dimensions || null
	};
}
function $u(e, t, n = null) {
	for (let r of qu(t)) {
		let t = vu(e.view, "video", e.config.labels?.videoUploading || "Uploading video…", n ?? e.view.state.selection.from);
		Qu(e, r, (n) => {
			yu(e.view, t, n);
		}).then((r) => Zu(e, r, {
			position: n,
			...
			xu(e.view, t), window.alert?.(n.message);
		});
	}
}
//#endregion
...
function td(e) {
	return Pu([e]).length ? "image" : Ku(e) ? "video" : null;
}
function nd(e) {
	let t = Number(e);
	if (!Number.isFinite(t) || t < 1) return "0Byte";
	let n = [
		"Byte",
		"KB",
		"MB",
		"GB"
	], r = Math.min(n.length - 1, Math.floor(Math.log(t) / Math.log(1024))), i = t / 1024 ** r;
	return `${i >= 10 || r === 0 ? Math.round(i) : i.toFixed(1)}${n[r]}`;
}
function rd(e) {
...
function id(e) {
	return {
		...e,
		download_url: su(e.download_url),
		source_filename: su(e.source_filename),
		thumbnail_filename: lu(e.thumbnail_filename)
	};
}
function ad(e) {
	let t = e.querySelector(".xefu-file-name")?.textContent?.trim() || "";
	return /^oembed_/i.test(t);
}
var od = class {
	constructor(e) {
		this.bridge = e, this.labels = {
			...ed,
			...e.config.labels || {}
		}, this.uploads = /* @__PURE__ */ new WeakMap(), this.fileEntries = /* @__PURE__ */ new WeakMap(), this.activeEntries = /* @__PURE__ */ new Set(), this.completedEntries = /* @__PURE__ */ new Set(), this.pendingFiles = /* @__PURE__ */ new Map(), this.container = e.form.querySelector(`#xefu-container-${e.sequence}`), this.container && (this.disableLegacyMediaInsertion(), this.decorate(), this.bindUploader());
	}
	refresh(e = 0) {
		if (!this.container || !window.jQuery) return;
		let t = window.jQuery(this.container), n = t.data("xefu-instance") || t.data("instance") || t.data();
		typeof n?.loadFilelist == "function" ? n.loadFilelist(t) : e < 20 && window.setTimeout(() => this.refresh(e + 1), 100);
	}
	disableLegacyMediaInsertion() {
		...
		this.autoinsertTypes = { ...e }, this.autoinsertPosition = this.container.dataset.autoinsertPosition === "inline" ? "inline" : "paragraph", e.image = !1, e.video = !1, this.container.dataset.autoinsertTypes = JSON.stringify(e), window.jQuery?.(this.container).data("autoinsertTypes", e);
	}
	decorate() {
		let e = [
			"auto",
			"light",
			"dark"
		].includes(this.bridge.config.colorset) ? this.bridge.config.colorset : "light";
		this.container.classList.add("roundeditor__attachments", `roundeditor__attachments--${e}`);
		let t = document.createElement("div");
		t.className = "roundeditor__attachments-heading";
		let n = document.createElement("strong");
		n.textContent = this.labels.attachments;
		let r = document.createElement("span");
		r.className = "roundeditor__attachments-help", r.textContent = this.labels.attachmentsHelp;
		let i = document.createElement("div");
		i.className = "roundeditor__attachments-actions", i.hidden = !0, t.append(n, r, i), this.headingActions = i, this.container.prepend(t), this.decorateDropzone(), this.mergeControlsIntoHeading(), this.addDropOverlay();
		let a = this.container.querySelector(".xefu-list");
		a && window.MutationObserver && (this.listObserver = new MutationObserver(() => {
			this.syncUploadPreviews(), this.decorateVideoItems(), this.decorateCoverButtons(), this.refreshUsageState(), this.syncLayout();
		}), this.listObserver.observe(a, {
			childList: !0,
			subtree: !0,
			attributes: !0,
		...
		}), this.decorateVideoItems(), this.decorateCoverButtons(), this.refreshUsageState(), this.syncLayout();
	}
	bindMediaSelectionFallback() {
		this.container.addEventListener("mousedown", (e) => {
			...
			let n = !!e.target.closest?.("input[type=\"checkbox\"]");
			t.dataset.roundeditorUploadPreview || (e.stopPropagation(), this.pendingMediaSelection = {
				item: t,
			...
			let r = !!e.target.closest?.("input[type=\"checkbox\"]");
			t.dataset.roundeditorUploadPreview || (r && e.preventDefault(), this.selectMediaItem(n?.item === t ? n : {
				item: t,
				selected: t.classList.contains("selected"),
				shiftKey: e.shiftKey,
				additive: !0,
				...
				this.setMediaItemSelected(t, t.classList.contains("selected")), this.syncLegacySelection(), this.syncSelectionActions();
			}, 0));
		}, !0);
	}
	...
	decorateDropzone() {
		...
		r && (r.replaceChildren(), r.append(q("upload"), document.createTextNode(this.labels.attachmentsSelectFile))), e.querySelector(".upload_info")?.classList.add("roundeditor__dropzone-hint");
	...
	mergeControlsIntoHeading() {
		...
		for (let [e, r, i] of n) {
			...
			a.type = "button", a.className = `xefu-btn roundeditor__attachment-action roundeditor__attachment-action--${i}`, a.hidden = !0, a.append(q(r), document.createElement("span")), a.lastElementChild.textContent = n.value || n.textContent || "", a.addEventListener("click", () => {
	...
	decorateCoverButtons() {
		for (let e of this.container.querySelectorAll(".xefu-act-set-cover")) e.querySelector(".roundeditor__attachment-icon") || e.replaceChildren(q("cover")), e.type = "button", e.classList.contains("roundeditor__thumbnail-checkbox") || e.classList.add("roundeditor__thumbnail-checkbox"), e.setAttribute("role", "checkbox"), e.setAttribute("aria-checked", String(e.closest(".xefu-file")?.classList.contains("xefu-is-cover-image")));
		for (let e of this.container.querySelectorAll(".xefu-file-video-play")) e.querySelector(".roundeditor__attachment-icon") || e.replaceChildren(q("play"));
		this.refreshImageCoverViews();
	}
	findFileItem(e) {
		let t = String(e || "");
		return !t || !this.container ? null : Array.from(this.container.querySelectorAll(".xefu-file")).find((e) => String(e.dataset.fileSrl || e.querySelector("[data-file-srl]")?.dataset.fileSrl || "") === t) || null;
	}
	isCover(e) {
		return !!this.findFileItem(e)?.classList.contains("xefu-is-cover-image");
	}
	toggleCover(e) {
		this.findFileItem(e)?.querySelector(".xefu-act-set-cover")?.click();
	}
	refreshImageCoverViews() {
		for (let e of this.bridge.imageViews || []) e.refreshCoverState();
	}
	refreshUsageState() {
		if (!this.container || !this.bridge.view) return;
		let e = /* @__PURE__ */ new Set();
		this.bridge.view.state.doc.descendants((t) => {
			t.attrs?.fileSrl && e.add(String(t.attrs.fileSrl));
		});
		for (let t of this.container.querySelectorAll(".xefu-list-images .xefu-file")) {
			if (t.dataset.roundeditorUploadPreview) continue;
			let n = String(t.dataset.fileSrl || t.querySelector("[data-file-srl]")?.dataset.fileSrl || ""), r = !!(n && !e.has(n) && !ad(t));
			t.classList.toggle("roundeditor__attachment--unused", r);
			let i = t.querySelector(".xefu-file-info") || t, a = t.querySelector(".roundeditor__attachment-unused-overlay");
			a || (a = document.createElement("span"), a.className = "roundeditor__attachment-unused-overlay", a.setAttribute("role", "img"), a.setAttribute("aria-label", this.labels.attachmentNotUsed), a.title = this.labels.attachmentNotUsed), a.parentElement !== i && i.appendChild(a), a.hidden = !r;
		}
	}
	decorateVideoItems() {
		let e = this.container?.querySelector(".xefu-list-images ul"), t = this.container?.querySelector(".xefu-list-files ul");
		if (!e || !t) return;
		let n = [...e.querySelectorAll("li"), ...t.querySelectorAll("li")];
		for (let r of n) {
			if (r.dataset.roundeditorUploadPreview) continue;
			let n = r.querySelector(".xefu-file-name")?.textContent?.trim() || "";
			if (!/\.(?:mp4|webm|mov)$/i.test(n) || r.dataset.roundeditorVideo) continue;
			r.dataset.roundeditorVideo = "true";
			let i = r.querySelector(".xefu-file-info") || r, a = this.pendingFiles.get(n), o = r.dataset.fileSrl || r.querySelector("[data-file-srl]")?.dataset.fileSrl || "", s = Number(r.dataset.duration), c = s > 0 ? "" : a && window.URL?.createObjectURL ? window.URL.createObjectURL(a) : o ? cu(`/index.php?module=file&act=procFileDownload&file_srl=${encodeURIComponent(o)}`) : "", l;
			if (r.parentElement === t) {
				r.classList.add("xefu-file-image"), r.classList.add("roundeditor__video-fallback");
				let t = c ? document.createElement("video") : document.createElement("span");
				t.className = "xefu-thumbnail", t.setAttribute("aria-hidden", "true"), t instanceof window.HTMLVideoElement && (t.src = c, t.muted = !0, t.playsInline = !0, t.preload = "metadata", l = t), i.insertBefore(t, i.firstChild), e.appendChild(r);
			} else c && (l = document.createElement("video"), l.className = "roundeditor__attachment-video-metadata", l.setAttribute("aria-hidden", "true"), l.tabIndex = -1, l.preload = "metadata", l.muted = !0, l.src = c, r.appendChild(l));
			if (!r.querySelector(".xefu-file-video")) {
				let e = document.createElement("span");
				e.className = "xefu-file-video", e.setAttribute("aria-hidden", "true");
				let t = document.createElement("span");
				t.className = "xefu-file-video-play", t.appendChild(q("play")), e.appendChild(t), r.appendChild(e);
			}
			s > 0 ? this.decorateVideoDuration(r, null, s) : l && this.decorateVideoDuration(r, l);
		}
	}
	decorateVideoDuration(e, t, n = 0) {
		let r = document.createElement("span");
		r.className = "roundeditor__attachment-video-duration", r.hidden = !0, e.appendChild(r);
		let i = (e) => {
			let t = Ju(e);
			r.textContent = t, r.hidden = !t, t ? r.setAttribute("aria-label", `${this.labels.videoDuration}: ${t}`) : r.removeAttribute("aria-label");
		};
		n > 0 ? i(n) : t && (t.addEventListener("loadedmetadata", () => i(t.duration), { once: !0 }), t.readyState >= 1 ? i(t.duration) : t.load());
	}
	bindUploader(e = 0) {
		let t = window.jQuery;
		if (!t) {
	...
	createUploadPreview(e) {
		let t = this.container.querySelector(".xefu-list-images ul");
		if (!t) return;
		let n = document.createElement("li");
		n.className = "xefu-file xefu-file-image roundeditor__attachment-upload", n.dataset.roundeditorUploadPreview = "true", n.setAttribute("aria-busy", "true");
		let r = document.createElement("strong");
		r.className = "xefu-file-name", r.textContent = e.file.name;
		let i = document.createElement("span");
		i.className = "xefu-file-info";
		let a = document.createElement("span");
		a.className = "xefu-file-size", a.textContent = nd(e.file.size);
		let o = document.createElement("span"), s = window.URL?.createObjectURL?.(e.file) || "";
		e.previewUrl = s;
		let c;
		if (e.type === "video" && s) {
			c = document.createElement("video"), c.muted = !0, c.playsInline = !0, c.preload = "metadata", c.src = s;
			let e = document.createElement("span");
			e.className = "xefu-file-video", e.setAttribute("aria-hidden", "true");
			let t = document.createElement("span");
			t.className = "xefu-file-video-play", t.appendChild(q("play")), e.appendChild(t), o.appendChild(e);
		} else c = document.createElement("span"), s && (c.style.backgroundImage = `url("${s.replaceAll("\"", "\\\"")}")`);
		c.className = "xefu-thumbnail", c.setAttribute("aria-hidden", "true"), o.appendChild(c), i.append(a, o);
		let l = document.createElement("strong");
		l.className = "roundeditor__attachment-upload-percent", l.textContent = "0%";
		let u = document.createElement("span");
		u.className = "roundeditor__attachment-upload-progress", u.setAttribute("aria-hidden", "true");
		let d = document.createElement("span");
		u.appendChild(d);
		let f = document.createElement("button");
		f.type = "button", f.className = "roundeditor__attachment-upload-cancel", f.title = this.labels.cancelUpload, f.setAttribute("aria-label", `${e.file.name}: ${this.labels.cancelUpload}`), f.appendChild(q("cancel"));
		let p = document.createElement("span");
		p.textContent = this.labels.cancel, f.appendChild(p), f.addEventListener("click", (t) => {
			t.preventDefault(), t.stopPropagation(), this.cancel(e);
		}), n.append(r, i, l, u, f), t.appendChild(n), e.previewItem = n, this.updateUploadPreview(e, 0), this.syncLayout();
	}
	updateUploadPreview(e, t, n = null) {
		let r = e.previewItem;
		if (!r?.isConnected) return;
		let i = Math.round(Math.min(1, Math.max(0, Number(t) || 0)) * 100);
		r.style.setProperty("--roundeditor-attachment-progress", `${i}%`);
		let a = r.querySelector(".roundeditor__attachment-upload-percent");
		a && (a.textContent = `${i}%`), r.setAttribute("aria-label", `${e.file.name} ${i}%${n ? ` ${n}` : ""}`);
	}
	removeUploadPreview(e) {
		e.previewItem?.remove(), e.previewItem = null, e.previewUrl && window.URL?.revokeObjectURL?.(e.previewUrl), e.previewUrl = "";
	}
	syncUploadPreviews() {
		for (let e of [...this.activeEntries, ...this.completedEntries]) {
			let t = e.previewItem?.dataset.fileSrl;
			t && [...this.container.querySelectorAll(".xefu-file[data-file-srl]")].find((e) => !e.dataset.roundeditorUploadPreview && e.dataset.fileSrl === t) && (this.removeUploadPreview(e), this.completedEntries.delete(e));
		}
	}
	add(e) {
		let t = Array.from(e?.files || []).map((e) => {
			let t = td(e);
			if (!t) return null;
			let n = !!this.autoinsertTypes?.[t], r = t === "image" ? this.labels.imageUploading : this.labels.videoUploading;
			return {
				file: e,
				type: t,
				autoInsert: n,
				progressSeen: !1,
				placeholderId: n ? vu(this.bridge.view, t, r) : null
			};
		}).filter(Boolean);
		for (let n of t) n.data = e, this.pendingFiles.set(n.file.name, n.file), this.fileEntries.set(n.file, [n]), this.activeEntries.add(n), this.createUploadPreview(n);
	...
	wrapSubmit(e) {
		...
		e._roundeditorSubmitWrapped = !0, e.submit = (...n) => {
			...
			let r = t.apply(e, n);
			e._roundeditorRequest = r;
			for (let t of this.entriesFor(e)) t.request = r;
			return r?.done?.(() => this.done(e)), r?.fail?.(() => this.fail(e)), r;
		};
	}
	cancel(e) {
		if (!e || !this.activeEntries.has(e)) return;
		let t = e.data, n = this.entriesFor(t).filter((e) => this.activeEntries.has(e));
		for (let e of n) {
			let t = e.previewItem?.querySelector(".roundeditor__attachment-upload-cancel");
			t && (t.disabled = !0);
		}
		let r = e.request || t?._roundeditorRequest;
		t._roundeditorCancelled = !0, typeof r?.abort == "function" ? r.abort() : typeof t?.abort == "function" && t.abort(), this.fail(t);
	}
	progress(e) {
		let t = e?.total ? e.loaded / e.total : 0;
		for (let n of this.entriesFor(e)) n.progressSeen = !0, yu(this.bridge.view, n.placeholderId, t, t >= .99 ? this.processingLabel(n.type) : null), this.updateUploadPreview(n, t, t >= .99 ? this.processingLabel(n.type) : null);
	}
	progressAll(e) {
		let t = e?.total ? e.loaded / e.total : 0;
		for (let e of this.activeEntries) e.progressSeen || (yu(this.bridge.view, e.placeholderId, t, t >= .99 ? this.processingLabel(e.type) : null), this.updateUploadPreview(e, t, t >= .99 ? this.processingLabel(e.type) : null));
	}
	processingLabel(e) {
		return e === "image" ? this.labels.imageProcessing : this.labels.videoProcessing;
	}
	done(e) {
		let t = this.entriesFor(e), n = rd(e);
		if (!n) {
			this.fail(e);
			return;
		}
		let r = id(n), i = String(r.source_filename || ""), a = t.find((e) => e.file.name === i) || t[0];
		!a || !this.activeEntries.has(a) || (a.placeholderId && yu(this.bridge.view, a.placeholderId, 1, this.processingLabel(a.type)), this.updateUploadPreview(a, 1, this.processingLabel(a.type)), a.autoInsert && a.type === "image" ? Lu(this.bridge, [r], {
			placeholderId: a.placeholderId,
			insertionMode: this.autoinsertPosition
		}) : a.autoInsert && Zu(this.bridge, r, {
			placeholderId: a.placeholderId,
			insertionMode: this.autoinsertPosition
		}), this.activeEntries.delete(a), this.fileEntries.delete(a.file), this.pendingFiles.delete(a.file.name), a.previewItem && (a.previewItem.dataset.fileSrl = String(r.file_srl || "")), this.completedEntries.add(a), this.uploads.delete(a.data || e));
	}
	fail(e) {
		for (let t of this.entriesFor(e)) xu(this.bridge.view, t.placeholderId), this.removeUploadPreview(t), this.activeEntries.delete(t), this.fileEntries.delete(t.file), this.pendingFiles.delete(t.file.name), this.uploads.delete(t.data || e);
...
function dd(e, t, n) {
	return ud(e.marks, t.marks) && n.every((n) => ld(e.attrs[n]) === ld(t.attrs[n]));
}
var fd = [
	"stickerSrl",
	"fileSrl",
...
function md(e, t) {
	...
	return t.descendants((e, t) => {
		let a = null;
		e.type.name === "sticker" ? a = n.get(sd(e))?.find((t) => dd(t, e, fd)) : e.type.name === "video" && (a = r.get(cd(e))?.find((t) => dd(t, e, pd))), !(!a || e.eq(a)) && (i = i.setNodeMarkup(t, null, a.attrs, a.marks));
	}), i.doc;
}
function hd(e, t) {
	let n = md(e.state.doc, t);
...
function Ad(e) {
	return e === "basefont" || e === "param" || e === "br" || e === "wbr" ? new Set(Td[e]) : /* @__PURE__ */ new Set([...wd, ...Td[e] || []]);
}
function jd(e) {
	let t = kd(e.getAttribute("style")).map(([e, t]) => !Ed.has(e) || !t || /(?:expression\s*\(|javascript\s*:|behavior\s*:)/i.test(t) ? "" : `${e}:${t}`).filter(Boolean).join(";");
	t ? e.setAttribute("style", `${t};`) : e.removeAttribute("style");
}
function Md(e) {
	let t = e.tagName.toLowerCase(), n = Ad(t), r = (t === "div" || t === "img") && e.hasAttribute("editor_component");
	for (let i of Array.from(e.attributes)) {
		let a = i.name.toLowerCase(), o = i.value, s = a.startsWith("data-") && bd.has(t), c = r && !a.startsWith("on");
		...
		if ((Od.has(a) || s) && Dd.test(o.replace(/\s+/g, ""))) {
			e.removeAttribute(i.name);
			continue;
		}
...
function Nd(e, t = []) {
	let n = /* @__PURE__ */ new Set([...t, "class"]), r = Array.from(e.attributes).map((e) => e.name).filter((e) => e !== "class" && !e.startsWith("data-roundeditor-")), i = r.length ? { [Cd]: r } : {};
...
function J(e, t = {}) {
	let n = { ...e || {} }, r = Array.isArray(n[Cd]) ? n[Cd] : [];
	delete n[Cd];
	let i = { ...n };
...
function Pd(e, t, n = {}) {
	let r = { ...e || {} }, i = r[Cd];
	delete r[Cd];
	let a = new Map(kd(r.style));
	for (let [e, n] of Object.entries(t)) n == null || n === "" ? a.delete(e) : a.set(e, String(n));
	return a.size ? r.style = `${Array.from(a, ([e, t]) => `${e}:${t}`).join(";")};` : delete r.style, i && (r[Cd] = i), J(r, n);
}
//#endregion
...
function Id(e) {
	return {
		...
		toDOM: (t) => [
			"span",
			Pd(t.attrs.extra, { [e]: t.attrs.value }),
			0
		]
	};
}
var Ld = {
	...
	strong: Fd(["strong", "b"], "strong"),
	em: Fd(["em", "i"], "em"),
	underline: Fd(["u"], "u"),
	strike: Fd(["s", "strike"], "s"),
	code: Fd(["code"], "code"),
	sub: Fd(["sub"], "sub"),
	sup: Fd(["sup"], "sup"),
	fontSize: Id("font-size"),
	fontColor: Id("color"),
	bgColor: Id("background-color"),
...
}, Rd = "data-roundeditor-raw", zd = `div[${Rd}][data-roundeditor-kind="block"]`, Bd = `span[${Rd}][data-roundeditor-kind="inline"]`, Vd = `div[${Rd}][data-roundeditor-kind="component-block"]`, Hd = `span[${Rd}][data-roundeditor-kind="component-inline"]`, Ud = `div[${Rd}][data-roundeditor-kind="embed"]`;
function Wd(e) {
...
function qd(e) {
	return { html: Gd(e.getAttribute(Rd)) };
}
//#endregion
...
function Zd(e, t = null, n = null) {
	return {
		src: su(e.getAttribute("src") || ""),
		alt: n ?? (e.getAttribute("alt") || ""),
		caption: t ?? (e.getAttribute("data-rx-roundeditor-caption") || ""),
		...
		extra: Y(e, [
			...
			"data-file-srl",
			"editor_component",
			"data-rx-roundeditor-caption"
		])
	};
}
function Qd(e, t = e.attrs.alt) {
	return ["img", Pd(e.attrs.extra, {
...
var nf = Fc({
	...
	image: {
		...
		parseDOM: [{
			...
			getContent: () => a.empty,
			getAttrs: (e) => {
				let t = e.querySelector("img:not([data-rx-sticker])");
				return t ? Zd(t, e.hasAttribute("data-roundeditor-caption") && e.querySelector(".roundeditor-content-image__caption")?.textContent || "", e.getAttribute("data-roundeditor-alt")) : !1;
			}
		}, {
		...
		toDOM: (e) => {
			let t = String(e.attrs.caption || "").trim(), n = Qd(e, t || e.attrs.alt || "");
			return t ? [
				"span",
				{
					...
					"data-roundeditor-alt": e.attrs.alt || ""
				},
				n,
				[
					"span",
	...
	audio: {
		...
		toDOM: (e) => ["audio", Pd(e.attrs.extra, { width: e.attrs.displayWidth }, {
			src: e.attrs.src,
			type: e.attrs.type,
			preload: e.attrs.preload,
			"data-file-srl": e.attrs.fileSrl,
	...
	video: {
		...
		parseDOM: [{
			...
			getAttrs: (e) => {
				...
				return t ? ef(t, e.querySelector(".roundeditor-content-image__caption")?.textContent || "") : !1;
			}
		}, {
			tag: "video",
			getAttrs: (e) => ef(e)
		...
		toDOM: (e) => {
			let t = String(e.attrs.caption || "").trim(), n = tf(e);
			return t ? [
				"span",
				{
					class: "roundeditor-content-image",
	...
	embed: {
		...
		toDOM: (e) => Kd("div", "embed", e.attrs.html)
	},
	rhymixComponentBlock: {
		...
		toDOM: (e) => Kd("div", "component-block", e.attrs.html)
	},
	rhymixComponentInline: {
		...
		toDOM: (e) => Kd("span", "component-inline", e.attrs.html)
	},
	rawBlock: {
		group: "block",
		atom: !0,
		...
		toDOM: (e) => Kd("div", "block", e.attrs.html)
	},
	rawInline: {
		inline: !0,
		group: "inline",
		...
		toDOM: (e) => Kd("span", "inline", e.attrs.html)
	},
	table: {
		...nf.table,
		content: "tableRow+",
	...
	tableCell: nf.table_cell,
	tableHeader: nf.table_header,
	text: { group: "inline" }
}, af = new Set("a audio b blockquote br code em font h1 h2 h3 h4 h5 h6 hr i img li ol p pre s span strike strong sub\n    sup table tbody td th tr u ul video".split(/\s+/)), of = new Set("script style form input button select textarea canvas svg".split(" "));
function sf(e, t) {
	let n = t === "inline" || t === "component-inline", r = document.createElement(n ? "span" : "div");
	r.setAttribute(Rd, Wd(e.outerHTML)), r.setAttribute("data-roundeditor-kind", t), e.replaceWith(r);
}
function cf(e) {
	let t = e.parentNode;
	if (t) {
...
function lf(e) {
	for (let t of Array.from(e.querySelectorAll("*")).reverse()) {
		...
		of.has(e) ? t.remove() : yd.has(e) ? Md(t) : cf(t);
	}
}
function uf(e) {
...
function hf(e) {
	...
	return t === "a" ? e.hasAttribute("href") : t === "li" ? ["OL", "UL"].includes(e.parentElement?.tagName) : t === "tbody" ? e.parentElement?.tagName === "TABLE" : t === "tr" ? e.parentElement?.tagName === "TBODY" : t === "td" || t === "th" ? e.parentElement?.tagName === "TR" : t === "table" ? uf(e) : t === "ol" || t === "ul" ? df(e) : t === "pre" ? ff(e) : t === "audio" ? mf(e) : t !== "video" || pf(e);
}
function gf(e) {
	let t = e.tagName.toLowerCase();
	if (t === "span" && e.hasAttribute("data-roundeditor-video")) {
		let t = e.querySelector("video");
		if (!t) {
			e.remove();
			return;
		}
		let n = e.querySelector(".roundeditor-content-media__caption, .roundeditor-content-image__caption") || Array.from(e.children).find((e) => e !== t && e.tagName === "SPAN");
		n && t.setAttribute("data-rx-roundeditor-caption", n.textContent || ""), e.replaceWith(t), gf(t);
		return;
	}
	if (t === "span" && e.hasAttribute("data-roundeditor-image")) {
		let t = e.querySelector("img:not([data-rx-sticker])"), n = e.querySelector("video");
		if (!t && n) {
			let t = e.hasAttribute("data-roundeditor-caption") ? e.querySelector(".roundeditor-content-image__caption") : null;
			t && n.setAttribute("data-rx-roundeditor-caption", t.textContent || ""), e.replaceWith(n), gf(n);
			return;
		}
		if (!t) {
			e.remove();
		...
		let r = e.hasAttribute("data-roundeditor-caption") ? e.querySelector(".roundeditor-content-image__caption") : null, i = e.getAttribute("data-roundeditor-alt");
		i !== null && t.setAttribute("alt", i), r && t.setAttribute("data-rx-roundeditor-caption", r.textContent || ""), e.replaceWith(t), gf(t);
		return;
	}
	if (of.has(t)) {
		e.remove();
	...
	if (!yd.has(t)) {
		for (let t of Array.from(e.children)) gf(t);
		cf(e);
		return;
	}
	Md(e);
	let n = e.getAttribute("editor_component"), r = t === "img" && n === "image_link", i = t === "div" && n === "oembed", a = t === "div" && e.hasAttribute("data-oembed-url"), o = af.has(t) && hf(e);
	if (n && !r || a || !o) {
		if (!i) lf(e);
		else for (let t of e.querySelectorAll("p,h1,h2,h3,h4,h5,h6")) bf(t);
		sf(e, n ? xd.has(t) ? "component-block" : "component-inline" : a ? "embed" : xd.has(t) ? "block" : "inline");
		return;
	}
	for (let t of Array.from(e.children)) gf(t);
}
function _f(e) {
	...
	let t = e;
	return ["AUDIO", "VIDEO"].includes(t.tagName) && !t.hasAttribute("data-roundeditor-kind") ? !1 : t.getAttribute("data-roundeditor-kind") === "block" || t.getAttribute("data-roundeditor-kind") === "component-block" || t.getAttribute("data-roundeditor-kind") === "embed" || xd.has(t.tagName.toLowerCase());
}
function vf(e) {
	if (!Array.from(e.childNodes).some(_f)) return;
...
function Sf(e) {
	...
	for (let e of Array.from(t.content.children)) gf(e);
	for (let e of Array.from(t.content.querySelectorAll("p"))) vf(e);
	for (let e of Array.from(t.content.querySelectorAll("li,td,th,blockquote"))) xf(e, !0);
	xf(t.content, !1);
	for (let e of Array.from(t.content.querySelectorAll("p,h1,h2,h3,h4,h5,h6"))) bf(e);
	for (let e of Array.from(t.content.querySelectorAll("p"))) yf(e);
...
function wf(e) {
	...
	for (; t;) {
		...
		for (let n of Array.from(e.querySelectorAll("span[style]"))) {
			...
			let r = new Map(Cf(n.getAttribute("style")));
			for (let [t, n] of Cf(e.getAttribute("style"))) r.set(t, n);
...
function Ef(e) {
	for (let t of Array.from(e.querySelectorAll(`[${Rd}]`))) {
		let e = document.createElement("template");
		e.innerHTML = Gd(t.getAttribute(Rd)), t.replaceWith(...Array.from(e.content.childNodes));
	}
...
function Of(e) {
	for (let t of Array.from(e.querySelectorAll("[style]"))) {
		let e = Cf(t.getAttribute("style")).map(([e, n]) => `${e}:${t.matches(".roundeditor-content-image__caption, .roundeditor-content-media__caption") ? n.replace(/rgba?\(([^)]*)\)/gi, (e) => e.replace(/\s*,\s*/g, ",")) : n}`).join(";");
		e ? t.setAttribute("style", `${e};`) : t.removeAttribute("style");
	}
}
function kf(e) {
	let t = Array.from(Sd).join("|");
...
function jf(e, t) {
	...
	return n.appendChild($e.fromSchema(t).serializeFragment(e.content)), Tf(n), wf(n), Ef(n), Df(n), Of(n), Af(kf(n.innerHTML));
}
//#endregion
...
function Pf(e, t) {
	...
	return a.length ? s.style = `${a.map(([e, t]) => `${e}:${t}`).join(";")};` : (delete s.style, Array.isArray(s[Nf]) && (s[Nf] = s[Nf].filter((e) => e !== "style"))), Object.entries(s).some(([e, t]) => e !== Nf || Array.isArray(t) && t.length > 0) && o.push(t.marks.rawMark.create({
		...n.attrs,
		extra: s
	})), o.push(...i), t.text(e.text, l.setFrom(o));
}
function Ff(e, t) {
	return e.isText ? Pf(e, t) : e.isLeaf ? e : e.type.create(e.attrs, a.fromArray(e.content.content.map((e) => Ff(e, t))), e.marks);
}
function If(e, t) {
	return Ff(e, t);
}
function Lf(e, t) {
	return new d(a.fromArray(e.content.content.map((e) => Ff(e, t))), e.openStart, e.openEnd);
}
//#endregion
...
function Rf(e) {
	let t = document.createElement("template");
	return t.innerHTML = Sf(e), If(ze.fromSchema(X).parse(t.content, { preserveWhitespace: "full" }), X);
}
function zf(e) {
	let t = document.createElement("template");
	return t.innerHTML = Sf(e), Lf(ze.fromSchema(X).parseSlice(t.content, { preserveWhitespace: "full" }), X);
}
//#endregion
//#region src/oembed.js
var Bf = /^(?:https?:)?\/\/[^\s<>"]+$/i, Vf = "oembed:failed_hosts", Hf = 1e4, Uf = 2e3, Wf = /* @__PURE__ */ new Map(), Gf = new zn("roundeditor-oembed-placeholders");
function Kf(e, t, n) {
	return R.node(e, t, { class: "roundeditor__oembed-pending" }, { id: n });
}
function qf() {
	return new In({
		key: Gf,
		state: {
			...
			apply(e, t) {
				...
				for (let [r, i] of t) {
					let t = e.mapping.mapResult(i.from, 1), a = e.doc.nodeAt(t.pos);
					!t.deletedAcross && a?.type.name === "paragraph" && a.textContent === i.url && n.set(r, {
						from: t.pos,
						to: t.pos + a.nodeSize,
						url: i.url
					});
				...
				let r = e.getMeta(Gf);
				return r?.add && n.set(r.add.id, {
					from: r.add.from,
					to: r.add.to,
					url: r.add.url
				}), r?.remove && n.delete(r.remove.id), n;
			}
		},
		props: { decorations(e) {
			let t = [];
			for (let [n, r] of Gf.getState(e) || []) {
				let i = e.doc.nodeAt(r.from);
				i && r.to === r.from + i.nodeSize && t.push(Kf(r.from, r.to, n));
			}
			return z.create(e.doc, t);
		} }
	});
}
function Jf() {
	try {
		let e = JSON.parse(window.sessionStorage.getItem(Vf) || "{}"), t = Date.now(), n = !1;
		for (let [r, i] of Object.entries(e)) (typeof i != "number" || t - i > Hf) && (delete e[r], n = !0);
		return n && window.sessionStorage.setItem(Vf, JSON.stringify(e)), e;
	} catch {
		return {};
	}
}
function Yf(e) {
	if (e) try {
		let t = Jf();
		t[e] = Date.now(), window.sessionStorage.setItem(Vf, JSON.stringify(t));
	} catch {}
}
function Xf(e) {
	if (e) try {
		...
		delete t[e], window.sessionStorage.setItem(Vf, JSON.stringify(t));
	} catch {}
}
function Zf(e) {
	try {
		return new URL(e, window.location.href).host;
	} catch {
		return "";
	}
}
function Qf(e) {
	return e.config.csrfToken ? String(e.config.csrfToken) : window.Rhymix && typeof window.Rhymix.getCSRFToken == "function" ? String(window.Rhymix.getCSRFToken() || "") : String(window.rx_csrf_token || document.querySelector("meta[name=\"csrf-token\"]")?.content || window.csrf_token || "");
}
function $f(e, t) {
	let n = new URLSearchParams();
	for (let [e, r] of Object.entries(t)) r != null && r !== "" && n.append(e, String(r));
	n.append("mid", e.config.mid || window.current_mid || "");
	let r = Qf(e);
	return r && n.append("_rx_csrf_token", r), {
		body: n,
		token: r
	};
}
function ep(e, t, n) {
	let { body: r, token: i } = $f(e, n);
...
function tp(e, t) {
	return ep(e, "/index.php?module=oembed&act=procOembedFetch", {
		url: t,
		editor_sequence: e.sequence
	}).then((e) => e.ok ? e.json() : { kind: "fail" }).catch(() => ({ kind: "fail" }));
}
function np(e, t, n) {
	let r = n || t, i = Wf.has(r), a = (Wf.get(r) || Promise.resolve()).catch(() => {}).then(() => i ? new Promise((e) => window.setTimeout(e, Uf)) : null).then(() => tp(e, t)), o = a.finally(() => {
		Wf.get(r) === o && Wf.delete(r);
	});
	return Wf.set(r, o), a;
}
function rp(e, t) {
	!e.sequence || !t || ep(e, "/index.php?module=file&act=procFileDelete", {
		editor_sequence: e.sequence,
		file_srl: t
	}).catch(() => {});
}
function ip(e, t) {
	t && (e.rebindControls?.(), e.primaryInput && (e.primaryInput.value = String(t)), e.config.uploadTargetSrl = Number(t));
}
function ap(e) {
	let t = String(e?.getData?.("text/plain") || "").trim();
	return t && Bf.test(t) ? t : null;
}
function op(e, t) {
	let { state: n } = e.view, r = n.schema.marks.link.create({ href: t }), i = n.schema.nodes.paragraph.create(null, n.schema.text(t, [r])), o = new d(a.from(i), 0, 0), s = {}, c = n.tr.replaceSelection(o), l = null;
	return c.doc.descendants((e, t) => e !== i || (l = t, !1)), l === null ? null : (c.setMeta(Gf, { add: {
		id: s,
		from: l,
		to: l + i.nodeSize,
		url: t
	} }), e.view.dispatch(c.scrollIntoView()), s);
}
function sp(e, t, n) {
	let r = Gf.getState(e)?.get(t);
	if (!r) return null;
	let i = e.doc.nodeAt(r.from);
	return i?.type !== e.schema.nodes.paragraph || i.textContent !== n ? null : r;
}
function cp(e, t) {
	e.view.dispatch(e.view.state.tr.setMeta(Gf, { remove: { id: t } }));
}
function lp(e, t, n, r) {
	let { state: i } = e.view, o = sp(i, t, n);
	if (!o) return !1;
	let s = Rf(r).content;
	if (!s.childCount) return !1;
	let c = [...s.content];
	if (c.some((e) => !e.isBlock)) return !1;
	c.push(i.schema.nodes.paragraph.create());
	let l = new d(a.fromArray(c), 0, 0), u = i.tr.replace(o.from, o.to, l).setMeta(Gf, { remove: { id: t } });
	return e.view.dispatch(u.scrollIntoView()), !0;
}
function up(e, t) {
	if (!e.config.oembedAvailable || typeof window.fetch != "function") return !1;
	let n = ap(t.clipboardData);
	if (!n) return !1;
	let r = Zf(n);
	if (r && Jf()[r]) return !1;
	t.preventDefault();
	let i = op(e, n);
	return i && np(e, n, r).then((t) => {
		if ((t.kind === "embed" || t.kind === "card") && t.wrapped_html) {
			lp(e, i, n, t.wrapped_html) ? (Xf(r), ip(e, t.upload_target_srl), t.file_srl && e.attachments?.refresh()) : t.file_srl && rp(e, t.file_srl);
			return;
		}
		r && !t.provider && Yf(r), cp(e, i);
	}), !0;
}
//#endregion
...
function fp(e, t) {
	...
	r.style.position = "fixed", r.style.top = "-10000px", r.style.left = "0", r.style.visibility = "hidden", r.style.whiteSpace = "pre", r.style.font = n.font, r.style.fontKerning = n.fontKerning, r.style.fontVariant = n.fontVariant, r.style.letterSpacing = n.letterSpacing, r.style.textTransform = n.textTransform, r.textContent = t, document.body.appendChild(r);
	let i = r.getBoundingClientRect().width;
	return r.remove(), i;
}
function pp(e, t, n) {
	let r = [0], i = 0;
	for (let t of String(e || "")) i += t.length, r.push(i);
	let a = 0, o = Math.abs(t);
	for (let i of r.slice(1)) {
		let r = Math.abs(n(String(e || "").slice(0, i)) - t);
		r >= o || (a = i, o = r);
	}
	return a;
}
function mp(e, t, n = (t) => fp(e, t)) {
	let r = window.getComputedStyle(e), i = e.getBoundingClientRect(), a = Number.parseFloat(r.borderLeftWidth) || 0, o = Number.parseFloat(r.borderRightWidth) || 0, s = Number.parseFloat(r.paddingLeft) || 0, c = Number.parseFloat(r.paddingRight) || 0, l = Math.max(0, i.width - a - o - s - c), u = n(e.value), d = i.left + a + s - e.scrollLeft;
	return u < l && (r.textAlign === "center" ? d += (l - u) / 2 : (r.textAlign === "right" || r.textAlign === "end") && (d += l - u)), pp(e.value, t - d, n);
}
function hp(e) {
...
var gp = class {
	...
	currentSize() {
		return {
			width: hp(this.node.attrs.displayWidth || this.node.attrs.width || this.media.width),
			height: hp(this.node.attrs.displayHeight || this.node.attrs.height || this.media.height)
		};
	}
	maxWidth() {
		let e = this.dom.closest(".ProseMirror");
		if (!e) return 640;
		let t = window.getComputedStyle(e), n = hp(t.paddingLeft) + hp(t.paddingRight), r = e.clientWidth || e.getBoundingClientRect().width;
		return Math.max(dp, r > n ? r - n : 640);
	}
	previewSize(e, t) {
		this.media.style.width = `${Math.round(e)}px`, this.media.style.height = `${Math.round(t)}px`, this.media.style.aspectRatio = `${e} / ${t}`, this.dom.style.width = `${Math.round(e)}px`;
	}
	startResize(e, t) {
		if (e.button !== 0) return;
		...
		let r = this.media.getBoundingClientRect(), i = this.currentSize(), a = r.width || i.width || dp, o = r.height || i.height || dp, s = a / o || 1, c = e.clientX, l = e.clientY, u = t.endsWith("e") ? 1 : -1, d = t.startsWith("s") ? 1 : -1, f = a, p = o, m = !1, h = (e) => {
			m || (e.preventDefault(), f = Math.min(this.maxWidth(), Math.max(dp, a + (e.clientX - c) * u)), p = e.altKey ? Math.max(dp, o + (e.clientY - l) * d) : Math.max(dp, f / s), this.previewSize(f, p));
		}, g = () => {
	...
	updateSize(e, t) {
		let n = this.position();
		if (n === null) return;
		let r = Math.round(Math.min(this.maxWidth(), Math.max(dp, e))), i = Math.round(Math.max(dp, t)), a = this.view.state.tr.setNodeMarkup(n, null, {
			...this.node.attrs,
			width: r,
			height: i,
			displayWidth: `${r}px`,
	...
	placeInputCaretAtPointer(e, t) {
		if (e.button !== void 0 && e.button !== 0 || e.isPrimary === !1) return;
		e.preventDefault(), e.stopPropagation();
		try {
		...
		let n = mp(t, e.clientX);
		t.setSelectionRange(n, n, "none");
	}
	renderSize() {
		let e = hp(this.node.attrs.width), t = hp(this.node.attrs.height);
		this.media.style.width = this.node.attrs.displayWidth || (e ? `${e}px` : ""), this.media.style.height = this.node.attrs.displayHeight || (t ? `${t}px` : "");
		let n = hp(this.media.style.width), r = hp(this.media.style.height);
		this.media.style.aspectRatio = n && r ? `${n} / ${r}` : "", this.dom.style.width = this.media.style.width;
	}
	selectNode() {
	...
	stopEvent(e) {
		return !!e.target.closest(".roundeditor__media-toolbar, .roundeditor__media-handle, .roundeditor__image-cover, .roundeditor__image-caption, .roundeditor__image-edge");
	}
	ignoreMutation() {
		return !0;
	}
...
function Z(e, t, n = e) {
	...
	return r.type = "button", r.dataset.mediaAction = e, r.title = t, r.setAttribute("aria-label", t), r.appendChild(Uu(n)), r;
}
function _p(e, t) {
	return Z(e, t, `align${e[0].toUpperCase()}${e.slice(1)}`);
}
function vp(e, t, n = "text") {
...
var yp = class {
	constructor({ labels: e, values: t, onDelete: n, onSize: r, onResetSize: i, onLink: a, onAlt: o, onAlign: s }) {
		this.labels = e, this.values = t, this.handlers = {
			...
			onAlt: o,
			onAlign: s
		}, this.element = document.createElement("div"), this.element.className = "roundeditor__media-toolbar", this.element.hidden = !0, this.element.contentEditable = "false", this.row = document.createElement("div"), this.row.className = "roundeditor__media-toolbar-row", this.row.append(Z("delete", e.imageDelete), Z("size", e.imageSize, "resize"), Z("link", e.imageLink), Z("alt", e.imageAlt), _p("left", e.alignLeft), _p("center", e.alignCenter), _p("right", e.alignRight)), this.formHost = document.createElement("div"), this.formHost.className = "roundeditor__media-toolbar-form", this.element.append(this.row, this.formHost), this.element.addEventListener("click", (e) => this.execute(e));
	}
	show() {
	...
	openForm(e) {
		...
		if (e === "size") {
			let e = vp(this.labels.imageWidth, "width", "number"), r = vp(this.labels.imageHeight, "height", "number");
			...
			let e = vp(this.labels.url, "href", "url");
			...
			let e = vp(this.labels.imageAlt, "alt");
	...
	constructor({ labels: e, values: t, onDelete: n, onSize: r, onResetSize: i, onToggleAutoplay: a, onToggleControls: o, onAlign: s }) {
		this.labels = e, this.values = t, this.handlers = {
			...
			onToggleControls: o,
			onAlign: s
		}, this.element = document.createElement("div"), this.element.className = "roundeditor__media-toolbar", this.element.hidden = !0, this.element.contentEditable = "false", this.row = document.createElement("div"), this.row.className = "roundeditor__media-toolbar-row", this.row.append(Z("delete", e.videoDelete), Z("size", e.videoSize, "resize"), Z("autoplay", e.videoAutoplay, "play"), Z("controls", e.videoControls), _p("left", e.alignLeft), _p("center", e.alignCenter), _p("right", e.alignRight)), this.formHost = document.createElement("div"), this.formHost.className = "roundeditor__media-toolbar-form", this.element.append(this.row, this.formHost), this.element.addEventListener("click", (e) => this.execute(e));
	}
	show() {
	...
	openSizeForm() {
		let e = this.values(), t = document.createElement("form"), n = vp(this.labels.videoWidth, "width", "number"), r = vp(this.labels.videoHeight, "height", "number");
	...
	constructor({ labels: e, values: t, onDelete: n, onSource: r, onToggleAutoplay: i, onToggleControls: a, onToggleLoop: o, onToggleMuted: s }) {
		this.labels = e, this.values = t, this.handlers = {
			onDelete: n,
			onSource: r,
			onToggleAutoplay: i,
			...
			onToggleLoop: o,
			onToggleMuted: s
		}, this.element = document.createElement("div"), this.element.className = "roundeditor__media-toolbar", this.element.hidden = !0, this.element.contentEditable = "false", this.row = document.createElement("div"), this.row.className = "roundeditor__media-toolbar-row", this.row.append(Z("delete", e.audioDelete), Z("source", e.audioSource, "link"), Z("autoplay", e.audioAutoplay, "play"), Z("controls", e.audioControls), Z("loop", e.audioLoop, "redo"), Z("muted", e.audioMuted, "audio")), this.formHost = document.createElement("div"), this.formHost.className = "roundeditor__media-toolbar-form", this.element.append(this.row, this.formHost), this.element.addEventListener("click", (e) => this.execute(e));
	}
	show() {
		this.element.hidden = !1, this.refresh();
	}
	...
	refresh() {
		...
		for (let t of [
			...
			"loop",
			"muted"
		]) this.row.querySelector(`[data-media-action="${t}"]`)?.setAttribute("aria-pressed", String(e[t]));
	}
	execute(e) {
		let t = e.target.closest("[data-media-action]")?.dataset.mediaAction;
		t && (e.preventDefault(), e.stopPropagation(), t === "delete" ? this.handlers.onDelete() : t === "source" ? this.openSourceForm() : t === "autoplay" ? this.handlers.onToggleAutoplay() : t === "controls" ? this.handlers.onToggleControls() : t === "loop" ? this.handlers.onToggleLoop() : t === "muted" && this.handlers.onToggleMuted(), this.refresh());
	}
	openSourceForm() {
		let e = document.createElement("form"), t = vp(this.labels.audioSource, "src");
		t.input.value = this.values().src || "", t.input.placeholder = "/path/to/audio.mp3", e.appendChild(t.wrapper), e.addEventListener("submit", (e) => {
			e.preventDefault();
			let n = t.input.value.trim();
			if (/^\s*(?:javascript|vbscript|data:text\/html):/i.test(n.replace(/\s+/g, ""))) {
				t.input.setCustomValidity(this.labels.invalidUrl), t.input.reportValidity();
				return;
			}
			t.input.setCustomValidity(""), this.handlers.onSource(n), this.formHost.replaceChildren();
		});
		let n = document.createElement("button");
		n.type = "submit", n.textContent = this.labels.apply, e.appendChild(n), this.formHost.replaceChildren(e), t.input.focus();
	}
}, Sp = {
...
var wp = class extends gp {
	constructor(e, t, n, r) {
		let i = document.createElement("img");
		i.draggable = !1, super(e, t, n, i), this.bridge = r, this.dom.classList.add("roundeditor__media--image"), this.labels = {
			...Sp,
			...r.config.labels || {}
		}, this.imageFrame = document.createElement("span"), this.imageFrame.className = "roundeditor__image-frame", this.imageFrame.append(this.media, ...this.handles), this.dom.appendChild(this.imageFrame), this.edge = document.createElement("button"), this.edge.type = "button", this.edge.className = "roundeditor__image-edge roundeditor__image-edge--top", this.edge.setAttribute("aria-label", this.labels.imageInsertParagraphBefore), this.edge.addEventListener("click", () => {
			let e = this.position();
			e !== null && ju(this.view, e) && this.refreshEdgeState();
		}), this.dom.prepend(this.edge), this.refreshEdgeOnPointerEnter = () => this.refreshEdgeState(), this.dom.addEventListener("pointerenter", this.refreshEdgeOnPointerEnter), this.cover = document.createElement("button"), this.cover.type = "button", this.cover.className = "roundeditor__image-cover", this.cover.setAttribute("role", "checkbox"), this.cover.append(q("cover"), document.createElement("span")), this.cover.lastElementChild.textContent = "대표", this.cover.addEventListener("click", () => this.bridge.attachments?.toggleCover(this.node.attrs.fileSrl)), this.imageFrame.appendChild(this.cover), this.caption = document.createElement("label"), this.caption.className = "roundeditor__image-caption", this.captionInput = document.createElement("input"), this.captionInput.type = "text", this.captionInput.draggable = !1, this.captionInput.autocomplete = "off", this.captionInput.placeholder = this.labels.imageCaptionPlaceholder, this.captionInput.setAttribute("aria-label", this.labels.imageCaption), this.caption.appendChild(this.captionInput), this.dom.appendChild(this.caption), this.captionEditing = !1, this.composingCaption = !1, this.captionInput.addEventListener("focus", () => {
			this.captionEditing = !0, this.renderCaption();
		}), this.captionInput.addEventListener("compositionstart", () => {
			this.composingCaption = !0;
		}), this.captionInput.addEventListener("compositionend", () => {
			this.composingCaption = !1, this.setCaptionFromInput();
		}), this.captionInput.addEventListener("input", () => {
			this.composingCaption || this.setCaptionFromInput();
		}), this.captionInput.addEventListener("blur", () => {
			this.captionEditing = !1, this.setCaption(this.captionInput.value.trim(), { select: !1 }), this.renderCaption();
		}), this.captionInput.addEventListener("pointerdown", (e) => this.placeInputCaretAtPointer(e, this.captionInput));
		for (let e of [
			"pointerdown",
			"mousedown",
			"click"
		]) this.captionInput.addEventListener(e, (e) => e.stopPropagation());
		this.captionInput.addEventListener("dblclick", (e) => {
			e.preventDefault(), e.stopPropagation(), this.captionInput.focus(), this.captionInput.select();
		}), this.captionInput.addEventListener("dragstart", (e) => e.preventDefault()), this.captionInput.addEventListener("keydown", (e) => {
			["Enter", "Escape"].includes(e.key) && (e.preventDefault(), this.setCaption(this.captionInput.value.trim(), { select: !1 }), this.captionInput.blur(), this.view.focus());
		}), r.imageViews?.add(this), this.toolbar = new yp({
			labels: this.labels,
			values: () => this.values(),
	...
	render() {
		this.media.src = this.node.attrs.src, this.media.alt = this.node.attrs.caption || this.node.attrs.alt || "", this.media.dataset.fileSrl = this.node.attrs.fileSrl || "", this.renderSize(), this.renderCaption(), this.refreshCoverState(), this.refreshEdgeState();
	}
	captionVisible() {
		return !!this.node.attrs.caption || this.captionEditing || this.dom.classList.contains("roundeditor__media--selected");
	}
	renderCaption() {
		let e = this.node.attrs.caption || "";
		!this.composingCaption && document.activeElement !== this.captionInput && this.captionInput.value !== e && (this.captionInput.value = e), this.caption.hidden = !this.captionVisible();
	}
	refreshCoverState() {
		let e = this.node.attrs.fileSrl, t = !!(e && this.bridge.attachments?.findFileItem(e)), n = t && this.bridge.attachments.isCover(e);
		this.cover.hidden = !t, this.cover.disabled = !t, this.cover.setAttribute("aria-checked", String(n)), this.cover.setAttribute("aria-label", n ? this.labels.imageCoverUnset : this.labels.imageCoverSet), this.cover.classList.toggle("roundeditor__image-cover--checked", n);
	}
	refreshEdgeState() {
		let e = this.position();
		e !== null && (this.edge.hidden = !Nu(this.view.state.doc, e));
	}
	update(e) {
		return e.type === this.node.type && (this.node = e, this.render(), !0);
	}
	selectNode() {
		super.selectNode(), this.renderCaption(), this.toolbar.show(), this.placeToolbar();
	}
	deselectNode() {
		super.deselectNode(), this.renderCaption(), this.toolbar.hide();
	}
	placeToolbar() {
		...
		let t = e.getBoundingClientRect(), n = this.imageFrame.getBoundingClientRect(), r = this.toolbar.element.getBoundingClientRect().height, i = n.top - t.top;
	...
	updateAttrs(e, t = this.node.marks, { select: n = !0 } = {}) {
		let r = this.position();
		if (r === null) return;
		let i = this.view.state.tr.setNodeMarkup(r, null, {
			...this.node.attrs,
			...e
		}, t);
		n && i.setSelection(T.create(i.doc, r)), this.view.dispatch(i);
	}
	setAlt(e, t = {}) {
		this.updateAttrs({ alt: String(e || "") }, this.node.marks, t);
	}
	setCaption(e, t = {}) {
		this.updateAttrs({ caption: String(e || "") }, this.node.marks, t);
	}
	setCaptionFromInput() {
		let e = this.captionInput.selectionStart, t = this.captionInput.selectionEnd, n = this.captionInput.selectionDirection;
		this.setCaption(this.captionInput.value, { select: !1 }), document.activeElement === this.captionInput && e !== null && t !== null && this.captionInput.setSelectionRange(e, t, n || "none");
	}
	setLink(e) {
		let t = Cp(e), n = this.view.state.schema.marks.link, r = this.node.marks.filter((e) => e.type !== n);
		t && r.push(n.create({
			href: t,
			target: "_blank",
	...
	destroy() {
		this.surface?.removeEventListener("scroll", this.placeToolbarOnScroll), this.dom.removeEventListener("pointerenter", this.refreshEdgeOnPointerEnter), this.bridge.imageViews?.delete(this);
	}
};
function Tp(e) {
	return (t, n, r) => new wp(t, n, r, e);
}
//#endregion
//#region src/nodeviews/AudioView.js
var Ep = {
	audio: "Audio",
	...
	constructor(e, t, n, r) {
		...
		i.draggable = !1, super(e, t, n, i), this.bridge = r, this.labels = {
			...Ep,
			...r.config.labels || {}
		}, this.dom.classList.add("roundeditor__media--audio"), this.suppressPlaybackClick = !1, this.selectForEditing = () => {
			let e = this.position();
			return e !== null && (this.view.dispatch(this.view.state.tr.setSelection(T.create(this.view.state.doc, e))), !0);
		}, this.selectBeforePlayback = (e) => {
			this.dom.classList.contains("roundeditor__media--selected") || this.selectForEditing() && (this.suppressPlaybackClick = !0, e.preventDefault(), e.stopImmediatePropagation());
		}, this.blockFirstPlayback = (e) => {
			let t = !this.dom.classList.contains("roundeditor__media--selected");
			!this.suppressPlaybackClick && !t || t && !this.selectForEditing() || (this.suppressPlaybackClick = !0, e.preventDefault(), e.stopImmediatePropagation(), e.type === "click" && (this.suppressPlaybackClick = !1));
		}, this.media.addEventListener("pointerdown", this.selectBeforePlayback, !0), this.media.addEventListener("mousedown", this.blockFirstPlayback, !0), this.media.addEventListener("click", this.blockFirstPlayback, !0), this.selectionCover = document.createElement("span"), this.selectionCover.className = "roundeditor__audio-selection-cover", this.selectionCover.contentEditable = "false", this.selectionCover.setAttribute("aria-hidden", "true"), this.selectFromCover = (e) => {
			e.preventDefault(), e.stopImmediatePropagation(), this.suppressPlaybackClick = !0, this.selectForEditing();
		}, this.selectionCover.addEventListener("mousedown", this.selectFromCover, !0), this.dom.appendChild(this.selectionCover), this.toolbar = new xp({
			labels: this.labels,
			values: () => this.values(),
			onDelete: () => this.remove(),
			onSource: (e) => this.updateAttrs({ src: su(e) }),
	...
	render() {
		...
		this.media.getAttribute("src") !== e && (this.media.pause(), e ? this.media.setAttribute("src", e) : this.media.removeAttribute("src"), this.media.load()), this.media.controls = !0, this.media.autoplay = !1, this.media.loop = !1, this.media.muted = !1, this.media.preload = "metadata", this.media.setAttribute("aria-label", this.labels.audio), this.media.dataset.fileSrl = this.node.attrs.fileSrl || "", this.dom.style.width = this.node.attrs.displayWidth || "", this.toolbar.refresh();
	}
	update(e) {
		return e.type === this.node.type && (this.node = e, this.render(), !0);
	}
	...
	destroy() {
		this.surface?.removeEventListener("scroll", this.placeToolbarOnScroll), this.media.removeEventListener("pointerdown", this.selectBeforePlayback, !0), this.media.removeEventListener("mousedown", this.blockFirstPlayback, !0), this.media.removeEventListener("click", this.blockFirstPlayback, !0), this.selectionCover.removeEventListener("mousedown", this.selectFromCover, !0), this.media.pause();
	}
};
function Op(e) {
	return (t, n, r) => new Dp(t, n, r, e);
}
//#endregion
...
var kp = { poll_maker: {
	...
	fallbackDetails: !1,
	resolveDetails: zp,
	pendingDetails: (e) => [Bp(e, e.componentPollLoading || "Loading…")],
	unavailableDetails: (e) => [Bp(e, e.componentPollUnavailable || "Unavailable")],
	invalidateDetails: Vp
} }, Ap = /* @__PURE__ */ new Map(), jp = /* @__PURE__ */ new Set([
	"class",
	"contenteditable",
	"editor_component",
	"height",
...
function Np(e, t, n, r = {}) {
	let i = kp[e] || {}, a = (i.details || []).flatMap((e) => {
	...
	return !a.length && n && i.fallbackDetails !== !1 && (a = Array.from(n.attributes).filter((e) => !jp.has(e.name) && !e.name.startsWith("data-roundeditor-")).slice(0, 3).map((e) => ({
		label: e.name,
		value: e.value
	}))), {
		name: e,
...
function Fp(e, t) {
	kp[e]?.invalidateDetails?.(t);
}
function Ip(e, t) {
	let n = Np(e, t, null);
	return n.icon ? Rp(n.icon, "roundeditor__component-icon") : null;
}
function Lp(e) {
	return e ? Rp(e, "roundeditor__component-card-icon") : null;
}
function Rp(e, t) {
	if (e.type === "ui") {
		let n = Uu(e.name);
...
function zp(e, t) {
	...
	return n ? Hp(n).then((e) => e.map((e) => Bp(t, e))) : Promise.resolve([]);
}
function Bp(e, t) {
	return {
...
var Up = "script,style,form,input,button,select,textarea,object,embed", Wp = /* @__PURE__ */ new Set([
	"href",
	"src",
	"poster",
	"data"
]), Gp = /* @__PURE__ */ new Map(), Kp = /* @__PURE__ */ new Map(), qp = /* @__PURE__ */ new Set(), Jp = /* @__PURE__ */ new Map([
	["https://platform.twitter.com/widgets.js", (e) => typeof window.twttr?.widgets?.load == "function" && (window.twttr.widgets.load(e), !0)],
	["https://www.instagram.com/embed.js", () => typeof window.instgrm?.Embeds?.process == "function" && (window.instgrm.Embeds.process(), !0)],
	["https://connect.facebook.net/ko_KR/sdk.js#xfbml=1&version=v18.0", (e) => typeof window.FB?.XFBML?.parse == "function" && (window.FB.XFBML.parse(e), !0)]
]);
function Yp(e, t) {
	let n = Jp.get(e.script);
	if (!n) return !1;
	try {
		return n(t);
	} catch {
...
function Xp(e, t) {
	for (let n of Array.isArray(t?.normalize) ? t.normalize : []) if (!(!n || typeof n.detect != "string" || typeof n.addClass != "string" || !n.addClass)) try {
		e.matches(n.detect) && e.classList.add(n.addClass);
		for (let t of e.querySelectorAll(n.detect)) t.classList.add(n.addClass);
	} catch {}
}
function Zp(e, t) {
	try {
...
function Qp(e, t = !1) {
	let n = e.script;
	if (!t && Gp.has(n)) return Gp.get(n);
	if (t && Kp.has(n)) return Kp.get(n);
	let r = new Promise((r, i) => {
		let a = t ? null : Array.from(document.scripts).find((e) => e.src === n);
		if (a?.dataset.roundeditorLoaded === "true") {
			qp.add(n), r();
			return;
		}
		a || (a = document.createElement("script"), a.async = !0, a.src = n, a.dataset.roundeditorOembedSdk = n, e.crossorigin === !0 && (a.crossOrigin = "anonymous"), (document.head || document.documentElement).appendChild(a)), a.addEventListener("load", () => {
			a.dataset.roundeditorLoaded = "true", qp.add(n);
			for (let e of document.querySelectorAll("script[data-roundeditor-oembed-sdk]")) e !== a && e.dataset.roundeditorOembedSdk === n && e.remove();
			r();
		}, { once: !0 }), a.addEventListener("error", () => i(/* @__PURE__ */ Error("The oEmbed SDK failed to load.")), { once: !0 });
	}).catch((e) => {
		throw Gp.delete(n), e;
	});
	return t ? (Kp.set(n, r), r.then(() => Kp.delete(n), () => Kp.delete(n))) : Gp.set(n, r), r;
}
function $p(e, t) {
	for (let n of Array.isArray(t) ? t : []) Xp(e, n);
	let n = (Array.isArray(t) ? t : []).filter((t) => t && typeof t.script == "string" && typeof t.selector == "string" && Zp(e, t.selector));
	n.length && Promise.resolve().then(() => {
		for (let t of n) Yp(t, e) || Qp(t, qp.has(t.script)).then(() => {
			Yp(t, e);
		}).catch(() => {});
	});
}
function em(e) {
	...
	if (!n || n.tagName !== "DIV" || n.getAttribute("editor_component") !== "oembed") return null;
	for (let e of n.querySelectorAll(Up)) e.remove();
	for (let e of [n, ...n.querySelectorAll("*")]) for (let t of Array.from(e.attributes)) {
		let n = t.name.toLowerCase();
		(n.startsWith("on") || n === "srcdoc" || Wp.has(n) && /^\s*(?:javascript|vbscript|data:text\/html)/i.test(t.value)) && e.removeAttribute(t.name);
	}
	return n.contentEditable = "false", n.classList.add("roundeditor__oembed"), n.dataset.roundeditorRawNode = "rhymixComponentBlock", n;
}
var tm = class {
	constructor(e, t) {
		if (this.node = e, this.dom = document.createElement(e.type.isInline ? "span" : "div"), this.dom.className = `roundeditor__raw roundeditor__raw--${e.type.isInline ? "inline" : "block"}`, this.dom.contentEditable = "false", this.dom.dataset.roundeditorRawNode = e.type.name, e.type.name.startsWith("rhymixComponent")) {
			if (Mp(e.attrs.html)?.getAttribute("editor_component") === "oembed") {
				let n = em(e.attrs.html);
				n && (this.dom = n, this.isOembed = !0, t?.config.oembedAvailable && $p(this.dom, t.config.oembedAssets));
				return;
			}
	...
	renderComponent(e) {
		let t = Mp(this.node.attrs.html), n = t?.getAttribute("editor_component") || "", r = e?.config.components?.[n], i = typeof r == "object" ? r.title : r, a = e?.config.labels || {}, o = Np(n, i, t, a);
		...
		let l = document.createElement("strong");
		l.textContent = o.title, c.appendChild(l), this.details = document.createElement("span"), this.details.className = "roundeditor__component-details", c.appendChild(this.details), this.renderDetails(o.details), o.resolveDetails && (this.renderDetails(o.pendingDetails), Pp(o, t, a).then((e) => {
...
var rm = {
	...
	videoCaptionPlaceholder: "Add a caption",
	...
	constructor(e, t, n, r) {
		...
		i.draggable = !1, i.tabIndex = -1, super(e, t, n, i), this.bridge = r, this.previewLoaded = !1, this.labels = {
			...rm,
			...r.config.labels || {}
		}, this.pausePlayback = () => this.media.pause(), this.showMetadata = () => {
			let e = Ju(this.media.duration);
			if (this.durationBadge.textContent = e, this.durationBadge.hidden = !e, this.durationBadge.setAttribute("aria-label", `${this.labels.videoDuration}: ${e}`), this.node.attrs.displayWidth || this.node.attrs.displayHeight || this.node.attrs.width || this.node.attrs.height) return;
			let t = this.media.videoWidth, n = this.media.videoHeight;
			if (!t || !n) return;
			let r = Math.min(t, this.maxWidth());
			this.previewSize(r, n / t * r);
		}, this.media.addEventListener("play", this.pausePlayback), this.media.addEventListener("loadedmetadata", this.showMetadata), this.dom.classList.add("roundeditor__media--video"), this.videoFrame = document.createElement("span"), this.videoFrame.className = "roundeditor__video-frame", this.videoFrame.append(this.media, ...this.handles), this.dom.appendChild(this.videoFrame), this.edge = document.createElement("button"), this.edge.type = "button", this.edge.className = "roundeditor__image-edge roundeditor__video-edge", this.edge.setAttribute("aria-label", this.labels.videoInsertParagraphBefore), this.edge.addEventListener("click", () => {
			let e = this.position();
			e !== null && ju(this.view, e) && this.refreshEdgeState();
		}), this.dom.prepend(this.edge), this.refreshEdgeOnPointerEnter = () => this.refreshEdgeState(), this.dom.addEventListener("pointerenter", this.refreshEdgeOnPointerEnter), this.playIndicator = document.createElement("span"), this.playIndicator.className = "roundeditor__video-play-indicator", this.playIndicator.contentEditable = "false", this.playIndicator.setAttribute("aria-hidden", "true"), this.playIndicator.appendChild(q("play")), this.durationBadge = document.createElement("span"), this.durationBadge.className = "roundeditor__video-duration", this.durationBadge.contentEditable = "false", this.durationBadge.hidden = !0, this.videoFrame.append(this.playIndicator, this.durationBadge), this.cover = document.createElement("button"), this.cover.type = "button", this.cover.className = "roundeditor__image-cover roundeditor__video-cover", this.cover.setAttribute("role", "checkbox"), this.cover.append(q("cover"), document.createElement("span")), this.cover.lastElementChild.textContent = "대표", this.cover.addEventListener("click", () => this.bridge.attachments?.toggleCover(this.node.attrs.fileSrl)), this.videoFrame.appendChild(this.cover), this.caption = document.createElement("label"), this.caption.className = "roundeditor__image-caption roundeditor__video-caption", this.captionInput = document.createElement("input"), this.captionInput.type = "text", this.captionInput.draggable = !1, this.captionInput.autocomplete = "off", this.captionInput.placeholder = this.labels.videoCaptionPlaceholder, this.captionInput.setAttribute("aria-label", this.labels.videoCaption), this.caption.appendChild(this.captionInput), this.dom.appendChild(this.caption), this.captionEditing = !1, this.composingCaption = !1, this.captionInput.addEventListener("focus", () => {
			this.captionEditing = !0, this.renderCaption();
		}), this.captionInput.addEventListener("compositionstart", () => {
			this.composingCaption = !0;
		}), this.captionInput.addEventListener("compositionend", () => {
		...
		this.captionInput.addEventListener("dblclick", (e) => {
			...
			["Enter", "Escape"].includes(e.key) && (e.preventDefault(), this.setCaption(this.captionInput.value.trim(), { select: !1 }), this.captionInput.blur(), this.view.focus());
		}), r.imageViews?.add(this), this.toolbar = new bp({
			labels: this.labels,
			values: () => this.values(),
			onDelete: () => this.remove(),
			onSize: (e, t) => this.resizeFromForm(e, t),
	...
	refreshCoverState() {
		let e = this.node.attrs.fileSrl, t = !!(e && this.bridge.attachments?.findFileItem(e)), n = t && this.bridge.attachments.isCover(e);
		this.cover.hidden = !t, this.cover.disabled = !t, this.cover.setAttribute("aria-checked", String(n)), this.cover.setAttribute("aria-label", n ? this.labels.videoCoverUnset : this.labels.videoCoverSet), this.cover.classList.toggle("roundeditor__image-cover--checked", n);
	}
	refreshEdgeState() {
		let e = this.position();
		e !== null && (this.edge.hidden = !Nu(this.view.state.doc, e));
	...
	loadPreview() {
		...
		let e = this.node.attrs.src || "";
		this.media.getAttribute("src") !== e && (this.durationBadge.hidden = !0, this.durationBadge.textContent = "", this.durationBadge.removeAttribute("aria-label"), this.media.pause(), e ? this.media.setAttribute("src", e) : this.media.removeAttribute("src"), this.media.load());
	}
	update(e) {
	...
	placeToolbar() {
		...
		if (!e || this.toolbar.element.hidden) return;
		e !== this.surface && (this.surface?.removeEventListener("scroll", this.placeToolbarOnScroll), this.surface = e, this.surface.addEventListener("scroll", this.placeToolbarOnScroll, { passive: !0 }));
		let t = e.getBoundingClientRect(), n = this.videoFrame.getBoundingClientRect(), r = this.toolbar.element.getBoundingClientRect().height;
		this.toolbar.element.classList.toggle("roundeditor__media-toolbar--below", n.top - t.top < r + 12);
	}
	values() {
		return {
	...
	updateAttrs(e, { select: t = !0 } = {}) {
		...
		t && r.setSelection(T.create(r.doc, n)), this.view.dispatch(r);
	}
	setCaption(e, t = {}) {
		this.updateAttrs({ caption: String(e || "") }, t);
	}
	setCaptionFromInput() {
		let e = this.captionInput.selectionStart, t = this.captionInput.selectionEnd, n = this.captionInput.selectionDirection;
		this.setCaption(this.captionInput.value, { select: !1 }), document.activeElement === this.captionInput && e !== null && t !== null && this.captionInput.setSelectionRange(e, t, n || "none");
	...
	destroy() {
		this.surface?.removeEventListener("scroll", this.placeToolbarOnScroll), this.observer?.disconnect(), this.dom.removeEventListener("pointerenter", this.refreshEdgeOnPointerEnter), this.bridge.imageViews?.delete(this), this.media.removeEventListener("play", this.pausePlayback), this.media.removeEventListener("loadedmetadata", this.showMetadata), this.media.pause();
	}
};
function am(e) {
	return (t, n, r) => new im(t, n, r, e);
}
//#endregion
//#region src/nodeviews/StickerView.js
var om = class extends gp {
	constructor(e, t, n, r) {
...
function lm(e) {
	let t = cm(e.form, "_saved_doc_title"), n = cm(e.form, "_saved_doc_content"), r = cm(e.form, "_saved_doc_message"), i = e.config.savedDocument, a = t?.value ?? i?.title ?? "", o = n?.value ?? i?.content ?? "";
...
var dm = /* @__PURE__ */ new Set(["rhymixComponentBlock", "rhymixComponentInline"]);
function fm(e) {
	let t = document.createElement("template");
	return dm.has(e.type.name) ? t.innerHTML = e.attrs.html : t.content.appendChild($e.fromSchema(X).serializeNode(e)), t.content.querySelector("[editor_component]")?.getAttribute("editor_component") || "";
}
function pm(e) {
	if (dm.has(e.type.name)) return e.attrs.html;
	let t = document.createElement("div");
...
function mm(e, t) {
	...
	for (let t of o) {
		...
		if (n && (dm.has(n.type.name) || n.attrs?.editorComponent)) return {
			node: n,
			position: t
		};
	}
...
function gm(e) {
	e.config.enableComponent && e.editable.addEventListener("dblclick", (t) => {
		...
		let i = document.createElement("div");
		i.className = "roundeditor__component-proxy", i.innerHTML = pm(n.node), e.wrapper.appendChild(i), window.editorPrevNode = i.firstElementChild, e.view.dispatch(e.view.state.tr.setSelection(T.create(e.view.state.doc, n.position)));
		let a = !1, o = null, s = new MutationObserver(() => {
			a || !i.isConnected || (a = !0, queueMicrotask(() => {
				if (a = !1, i.isConnected) try {
					Fp(r, i.querySelector("[editor_component]"));
					let t = zf(i.innerHTML), a = e.view.state.tr.replaceRange(n.position, n.position + n.node.nodeSize, t);
...
function vm(e) {
	return !!e && e.nodeType === Node.ELEMENT_NODE && xd.has(e.tagName.toLowerCase());
}
function ym(e) {
...
function Sm(e, t) {
	let n = "  ".repeat(t), r = e.tagName.toLowerCase(), i = Array.from(e.children).some(vm);
	if (Sd.has(r) || !i || ["pre", "code"].includes(r)) return [`${n}${e.outerHTML}`];
	let [a, o] = ym(e);
	return [
		`${n}${a}`,
		...xm(e, t + 1),
...
function Cm(e) {
	let t = document.createElement("template");
	return t.innerHTML = String(e || ""), xm(t.content, 0).join("\n");
}
function wm(e) {
	let t = Array.from(e.children || []).some(vm);
	for (let n of Array.from(e.childNodes)) n.nodeType === Node.TEXT_NODE ? /^[\t\n\r ]+$/u.test(n.nodeValue) && n.nodeValue.includes("\n") && (e.nodeType === Node.DOCUMENT_FRAGMENT_NODE || t || vm(n.previousSibling) || vm(n.nextSibling)) && n.remove() : n.nodeType === Node.ELEMENT_NODE && wm(n);
}
function Tm(e) {
	let t = document.createElement("template");
	return t.innerHTML = String(e || ""), wm(t.content), t.innerHTML;
}
//#endregion
...
var Em = class {
	...
	open() {
		!this.bridge.config.htmlMode || this.bridge.config.allowHtml === !1 || this.active || (this.bridge.toolbar.closePanel(), this.bridge.toolbar.closeMore(), this.textarea.value = Cm(this.bridge.serializeVisual()), this.bridge.surface.hidden = !0, this.textarea.hidden = !1, this.bridge.wrapper.classList.add("roundeditor--source"), window.editorMode[this.bridge.sequence] = "html", this.bridge.compat.mode = "html", this.textarea.focus(), this.bridge.toolbar.refresh(this.bridge.view.state), this.bridge.sync());
	}
	close() {
	...
	commit() {
		this.active && (this.bridge.updateDocument(Tm(this.textarea.value)), this.textarea.value = Cm(this.bridge.serializeVisual()));
	}
	getData() {
		return this.active ? Tm(this.textarea.value) : this.bridge.serializeVisual();
	}
	setData(e) {
		this.active ? this.textarea.value = Cm(e) : this.bridge.updateDocument(e);
	}
	insertHtml(e) {
...
function Dm(e, t = null) {
	return function(n, r) {
		...
		if (!o) return !1;
		let s = r ? n.tr : null;
		return Om(s, o, e, t) ? (r && r(s.scrollIntoView()), !0) : !1;
	};
}
...
function jm(e) {
	return function(t, n) {
		...
		return a ? n ? r.node(a.depth - 1).type == e ? Mm(t, n, e, a) : Nm(t, n, a) : !0 : !1;
	};
}
function Mm(e, t, n, r) {
...
var Fm = [
	...
	"1.8",
	"2"
], Lm = /* @__PURE__ */ "#000000.#434343.#666666.#999999.#b7b7b7.#cccccc.#ffffff.#980000.#ff0000.#ff9900.#ffff00.#00ff00.#00ffff.#4a86e8.#0000ff.#9900ff.#ff00ff.#e25041.#f6b26b.#ffd966.#93c47d.#76a5af.#6d9eeb.#674ea7.#c27ba0.#a61c00.#38761d".split(".");
function Q(e, t) {
...
function Bm(e, t, n) {
	return zm(e, t, n) || Am(e.schema.nodes.listItem)(e, t, n);
}
function Vm(e, t) {
...
function Jm(e) {
	return (t, n, r) => {
		if (Hm(t, t.schema.nodes.listItem) && (e > 0 ? Pm(t.schema.nodes.listItem) : jm(t.schema.nodes.listItem))(t, n, r)) return !0;
		let i = Km(t).filter((e) => {
			let n = t.doc.nodeAt(e)?.type;
			return n === t.schema.nodes.paragraph || n === t.schema.nodes.heading;
		});
...
function Xm(e) {
	return (t, n, r) => Hm(t, e.nodes.blockquote) ? Xn(t, n, r) : ur(e.nodes.blockquote)(t, n, r);
}
function Zm(e, t) {
	return (n, r, i) => Hm(n, e) ? jm(t)(n, r, i) : Dm(e)(n, r, i);
}
function Qm(e, t) {
	if (!t) return !0;
	let { from: n, to: r, empty: i } = e.selection, a = Km(e).filter((t) => {
...
function oh({ labels: e, onApply: t, onRemove: n, onClose: r }) {
...
function sh({ labels: e, operations: t, onInsert: n, onOperation: r, onClose: i }) {
...
function lh(e = {}) {
	return ch("getStickerPickerList", { mid: "sticker" }).then((e) => Array.isArray(e?.sticker) ? e.sticker : []);
}
function uh(e, t) {
	return ch("getStickerElemList", {
		mid: "sticker",
		sticker_srl: t
	}).then((e) => Array.isArray(e?.stickerImage) ? e.stickerImage : []);
}
function dh(e, t) {
	return t.length ? ch("resolveStickers", {
		mid: "sticker",
		stickers: JSON.stringify(t)
	}).then((e) => Array.isArray(e?.stickers) ? e.stickers : []) : Promise.resolve([]);
}
function fh(e, t = "") {
	let n = e.type === "video" ? "video" : "image", r = lu(e.poster || (n === "image" ? e.url : ""));
	return {
		stickerSrl: String(e.sticker_srl),
		fileSrl: String(e.sticker_file_srl),
		mediaType: n,
		src: r,
		videoSrc: n === "video" ? cu(e.url) : null,
...
function ph(e, t, n = "") {
	...
	let r = e.view.state.schema.nodes.sticker.create(fh(t, n)), i = e.view.state.tr.replaceSelectionWith(r);
...
async function mh(e) {
	...
	}), r = await dh(e.config, n), i = new Map(r.map((e) => [`${e.sticker_srl}|${e.sticker_file_srl}`, e])), a = e.view.state.tr;
	for (let [n, r] of t) {
		let t = i.get(n);
		if (t?.valid) for (let n of r) {
			...
			!r || r.type !== e.view.state.schema.nodes.sticker || (a = a.setNodeMarkup(n, null, {
				...
				src: lu(t.poster || t.url),
				videoSrc: t.type === "video" ? cu(t.url) : null,
...
var hh = "roundeditor.recentStickers", gh = 30;
function _h() {
	try {
		let e = JSON.parse(localStorage.getItem(hh) || "[]");
		return Array.isArray(e) ? e.slice(0, gh) : [];
	} catch {
		return [];
	}
}
function vh(e) {
	...
	}, n = [t, ..._h().filter((e) => String(e.sticker_srl) !== String(t.sticker_srl) || String(e.sticker_file_srl) !== String(t.sticker_file_srl))].slice(0, gh);
	try {
		localStorage.setItem(hh, JSON.stringify(n));
	} catch {}
}
function yh(e, t) {
	if (e.type === "video") {
...
function xh(e, t, n) {
	...
	let m = /* @__PURE__ */ new Map(), h = null, g = null, ee = (n, r = null) => {
		...
		try {
			if (!m.has(g)) {
				let t = n ? (await uh(e.config, n.sticker_srl)).map((e) => ({
					...e,
					sticker_srl: e.sticker_srl || n.sticker_srl
				})) : await dh(e.config, _h());
				m.set(g, t);
			}
		...
		let n = document.createElement("button");
		if (n.type = "button", n.className = "roundeditor__sticker-pack", n.setAttribute("role", "tab"), n.setAttribute("aria-label", t), n.title = t, e?.main_image || e?.poster) n.appendChild(bh(e));
		else {
			let e = document.createElement("span");
	...
	return s.appendChild(te), lh(e.config).then((e) => {
...
function Ch(e, t, n = {}) {
	let r = document.createElement("button");
	return r.type = "button", r.className = `roundeditor__tool${n.className ? ` ${n.className}` : ""}`, r.dataset.command = e, r.title = t[e] || e, r.setAttribute("aria-label", t[e] || e), r.appendChild(Uu(n.icon || e)), n.disabled && (r.disabled = !0, r.setAttribute("aria-disabled", "true")), r;
}
function wh(e, t, n = !1) {
	let r = document.createElement("button");
...
var Eh = class {
	...
	build() {
		...
		[
			...
			"underline",
			"strike"
		].forEach((t) => e.appendChild(Ch(t, this.labels))), [
			"fontSize",
			"lineHeight",
			"textColor",
			"backgroundColor"
		].forEach((t) => e.appendChild(Ch(t, this.labels))), e.appendChild(this.moreButton("text"));
		let t = this.addGroup("rich");
		if (t.appendChild(Ch("link", this.labels)), t.appendChild(this.moreButton("rich")), this.addGroup("paragraph").appendChild(this.moreButton("paragraph", "paragraph")), this.addGroup("sticker").appendChild(Ch("sticker", this.labels)), this.bridge.config.enableComponent) {
			let e = this.addGroup("components");
			for (let [t, n] of Object.entries(this.bridge.config.components || {})) {
				let r = typeof n == "object" ? n.title : n;
				e.appendChild(Th(t, r));
			}
		...
		[
			...
			"redo",
			"selectAll"
		].forEach((e) => r.appendChild(Ch(e, this.labels))), this.bridge.config.htmlMode && this.bridge.config.allowHtml !== !1 && r.appendChild(Ch("source", this.labels)), r.appendChild(Ch("fullscreen", this.labels)), r.appendChild(this.moreButton("right")), this.element.addEventListener("mousedown", (e) => {
	...
	moreButton(e, t = "more") {
		let n = Ch("more", this.labels, { icon: t });
	...
	moreTools(e) {
		...
		}[e] || []).filter((e) => e !== "source" || this.bridge.config.htmlMode && this.bridge.config.allowHtml !== !1).map((e) => e === "format" ? Ch("format", { format: this.labels.normal }) : Ch(e, this.labels));
	}
	observeWidth() {
		let e = (e) => {
			if (!Number.isFinite(e) || e <= 0) return;
			let t = e <= 720, n = e <= 480;
			(t !== this.compact || n !== this.narrow) && (this.compact = t, this.narrow = n, this.bridge.wrapper.classList.toggle("roundeditor--compact", t), this.bridge.wrapper.classList.toggle("roundeditor--narrow", n), this.activeMore && this.moreRow.replaceChildren(...this.moreTools(this.activeMore)));
		};
		typeof ResizeObserver == "function" && (this.resizeObserver = new ResizeObserver((t) => {
			let n = t[0];
			e(n?.contentRect?.width || this.bridge.wrapper.clientWidth);
		}), this.resizeObserver.observe(this.bridge.wrapper)), e(this.bridge.wrapper.getBoundingClientRect().width || this.bridge.wrapper.clientWidth);
	}
	closeMore() {
	...
	colorPanel(e, t) {
		...
		let r = wh(this.labels.reset, "");
		r.classList.add("roundeditor__swatch", "roundeditor__swatch--reset"), r.addEventListener("click", () => {
			Q(this.bridge.view, Gm(this.bridge.view.state.schema.marks[e], null)), this.closePanel();
		}), n.appendChild(r);
		for (let t of Lm) {
			let r = wh(t, t);
			r.classList.add("roundeditor__swatch"), r.style.setProperty("--roundeditor-swatch", t), r.setAttribute("aria-label", t), r.addEventListener("click", () => {
				Q(this.bridge.view, Gm(this.bridge.view.state.schema.marks[e], t)), this.closePanel();
			}), n.appendChild(r);
		}
	...
	execute(e) {
		...
		let t = this.bridge.view, { schema: n } = t.state, r = {
			bold: Wm(n.marks.strong),
			italic: Wm(n.marks.em),
			underline: Wm(n.marks.underline),
			strike: Wm(n.marks.strike),
			clearFormatting: Qm,
			alignLeft: qm({ align: null }),
			alignCenter: qm({ align: "center" }),
			alignRight: qm({ align: "right" }),
			alignJustify: qm({ align: "justify" }),
			orderedList: Zm(n.nodes.orderedList, n.nodes.listItem),
			bulletList: Zm(n.nodes.bulletList, n.nodes.listItem),
			outdent: Jm(-1),
			indent: Jm(1),
			quote: Xm(n),
			horizontalRule: nh,
		...
		if (e === "fontSize") {
			let r = this.choices(e, Fm, (e) => `${e}px`, (e) => Q(t, Gm(n.marks.fontSize, `${e}px`))), i = wh(this.labels.custom, "custom");
			...
			let r = [{
				...
				value: null
			}, ...Array.isArray(this.bridge.config.fontFamilies) ? this.bridge.config.fontFamilies : []], i = this.choices(e, r, (e) => e.label, (e) => Q(t, Gm(n.marks.fontFamily, e.value)));
			...
			let n = this.choices(e, Im, (e) => e === "1" ? this.labels.reset : e, (e) => Q(t, qm({ lineHeight: e === "1" ? null : e })));
			...
			let t = xh(this.bridge, this.labels, () => this.closePanel());
	...
	customFontSize() {
		...
		e.className = "roundeditor__panel-form", e.innerHTML = `<label class="roundeditor__field"><span>${this.labels.fontSize}</span><input type="number" min="1" max="300" value="15" inputmode="numeric"></label><button class="roundeditor__button roundeditor__button--primary" type="submit">${this.labels.apply}</button>`, e.addEventListener("submit", (t) => {
			...
			Q(this.bridge.view, Gm(this.bridge.view.state.schema.marks.fontSize, `${n}px`)), this.closePanel();
	...
	formatPanel() {
		...
		], t = this.choices("format", e, (e) => e[1], (e) => Q(this.bridge.view, Ym(e[0])));
	...
	linkPanel() {
		let e = oh({
			labels: this.labels,
			onApply: (e, t) => {
				Q(this.bridge.view, $m(e, t)), this.closePanel();
			},
			onRemove: () => {
				Q(this.bridge.view, eh), this.closePanel();
			},
	...
	tablePanel() {
		let e = [
			...
			operations: t,
			onInsert: (e, t) => {
				Q(this.bridge.view, th(e, t)), this.closePanel();
			},
			onOperation: (e) => {
	...
	characterPanel() {
		let e = this.choices("characters", [..."©®™…—–·•※★☆♥♡✓→←↑↓±×÷≠≤≥∞℃₩€¥£§¶「」『』【】"], (e) => e, (e) => Q(this.bridge.view, rh(e)));
	...
	refresh(e) {
		...
		for (let [t, n] of Object.entries({
			...
			let r = this.element.querySelector(`[data-command="${t}"]`), i = Vm(e, e.schema.marks[n]);
		...
		for (let [t, r] of Object.entries(n)) {
			let n = this.element.querySelector(`[data-command="${t}"]`), i = Hm(e, r);
...
function Ah(e) {
	for (let t of Array.isArray(e) ? e : []) {
		...
		if (Array.from(document.querySelectorAll("link[rel=\"stylesheet\"]")).some((e) => e.href === n)) continue;
		let r = document.createElement("link");
		r.rel = "stylesheet", r.href = e, r.dataset.roundeditorContentCss = "", document.head.appendChild(r);
	}
}
function jh(e, t) {
...
function Nh(e) {
	...
	return e.oembedAvailable && t.splice(5, 0, qf()), t;
}
function Ph(e, t, n) {
	if (!e.config.allowUpload || n) return !1;
	let r = Pu(t.dataTransfer?.files), i = qu(t.dataTransfer?.files);
	...
	return r.length && Ru(e, r, a), i.length && $u(e, i, a), !0;
}
function Fh(e, t) {
	if (e.sourceMode?.insertHtml(t)) return e.sourceMode.focus(), e.sync();
	let n = Rf(t), r = Array.from({ length: n.childCount }, (e, t) => n.child(t)).filter((e) => !(e.type === X.nodes.paragraph && e.attrs.unwrap && e.childCount === 0)), i = (e) => e.type === X.nodes.paragraph && e.childCount > 0 && Array.from({ length: e.childCount }, (t, n) => e.child(n)).every((e) => [
		"audio",
		"image",
		"video"
	].includes(e.type.name)), o = r.length > 0 && r.every(i), s = o ? new d(a.fromArray(r.flatMap((e) => Array.from({ length: e.childCount }, (t, n) => e.type.create({
		...e.attrs,
		unwrap: !1
	}, e.child(n), e.marks)))), 0, 0) : zf(t), c = e.view.state.tr.replaceSelection(s);
	if (o) {
		let e = [];
		c.doc.descendants((t, n) => {
			t.attrs.unwrap && i(t) && e.push(n);
		});
		for (let t of e) {
			let e = c.doc.nodeAt(t);
			c = c.setNodeMarkup(t, null, {
				...e.attrs,
				unwrap: !1
			});
		}
	}
	return e.view.dispatch(c), e.view.focus(), e.sync();
}
function Ih(e) {
	return e.sourceMode?.selectedHtml() ?? jf({ content: e.view.state.selection.content().content }, X);
}
function Lh(e, t = !1) {
...
function zh(e) {
	return {
		...
		insertHtml: (t) => Fh(e, t),
		getText: () => Lh(e),
		getSelection: () => ({ getSelectedText: () => Lh(e, !0) }),
		focus: () => e.sourceMode?.focus()
	};
}
function Bh() {
	window.RoundEditorGlobalsInstalled = !0, Dh || (Dh = {
	...
	window._getCkeInstance = (t) => {
		...
		let n = $[Oh(t)];
		return n ? Ih(n) : e.getSelected?.(t) || "";
	};
}
function Vh() {
	for (let e of Object.values($)) Hh(e);
}
function Hh(e) {
	Bh(), window.editorRelKeys = window.editorRelKeys || [], window.editorMode = window.editorMode || [], e.rebindControls(), window.editorRelKeys[e.sequence] = {
...
function Gh(e) {
	let t = kh(e);
	Ah(t.contentCss);
	let n = Oh(t.editorSequence || e.dataset.editorSequence), r = e.closest("form");
	if (!n || !r) throw Error("The editor sequence or parent form is missing.");
	let i = jh(r, t.contentKeyName);
	...
	let a = {
		...
		config: t,
		sequence: n,
		primaryInput: jh(r, t.primaryKeyName) || { value: "" },
		contentInput: i,
		view: null,
		editable: null,
		...
		fullscreen: null,
		attachments: null,
		imageViews: /* @__PURE__ */ new Set(),
		rebindControls() {
			let e = this.wrapper.closest("form") || this.form;
			this.form = e, this.primaryInput = jh(e, this.config.primaryKeyName) || this.primaryInput, this.contentInput = jh(e, this.config.contentKeyName) || this.contentInput;
		},
		sync() {
			return this.rebindControls(), this.view && (this.contentInput.value = this.sourceMode?.getData() ?? this.serializeVisual()), Hh(this), this.contentInput.value;
		},
		serializeVisual() {
			return jf(this.view.state.doc, X);
		},
		prepareSubmit() {
	...
	a.view = new zs(e.querySelector(".roundeditor__surface"), {
		...
		transformPastedHTML: Sf,
		handlePaste: (e, t) => zu(a, t) || up(a, t),
		handleDrop: (e, t, n, r) => Ph(a, t, r),
		nodeViews: {
			...nm(a),
			audio: Op(a),
			image: Tp(a),
		...
		dispatchTransaction(e) {
			a.view.updateState(a.view.state.apply(e)), a.sync(), a.attachments?.refreshUsageState(), a.toolbar?.refresh(a.view.state);
		}
	}), a.editable = a.view.dom, a.editable.editor_sequence = n, a.editable.setFocus = () => a.view.focus(), a.editable.replaceHTML = (e) => Fh(a, e), a.compat = zh(a), a.toolbar = new Eh(a), a.sourceMode = new Em(a), a.fullscreen = new _m(a), t.allowUpload && (a.attachments = new od(a)), a.toolbar.refresh(a.view.state), $[n] = a, r.setAttribute("editor_sequence", String(n)), Mh(r, "use_editor", "Y"), Mh(r, "use_html", "Y"), Uh(a), Vh(), gm(a), r.addEventListener("submit", () => a.prepareSubmit(), !0), a.sync(), um(a), mh(a).catch((e) => console.warn("[roundeditor] Sticker resolution failed.", e)), e.querySelector(".roundeditor__loading")?.remove(), e.classList.add("roundeditor--ready"), t.focus && a.view.focus();
}
function Kh() {
...
document.readyState === "loading" ? document.addEventListener("DOMContentLoaded", Kh, { once: !0 }) : Kh(), window.addEventListener("load", Vh), window.addEventListener("pageshow", () => {
	Kh(), Vh();
